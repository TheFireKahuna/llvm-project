...
static DWORD __stdcall mmap_worker(void *arg) {
...
//    to replace. Tests the FixedMapLock serialization and prepare_for_fixed
...
// ---------------------------------------------------------------------------
// 7. Concurrent MAP_FIXED on adjacent addresses — tests FixedMapLock
//    under real contention from multiple threads.
// ---------------------------------------------------------------------------
...
static DWORD __stdcall fixed_worker(void *arg) {
...
//    Exercises the retry-classify loop, FixedMapLock contention on the
//    same VA, and the TOCTOU recovery path where MBI changes between
//    query and dispatch.
// ---------------------------------------------------------------------------
...
static DWORD __stdcall same_addr_worker(void *arg) {
...
static DWORD __stdcall noreplace_claimer(void *arg) {
...
static DWORD __stdcall noreplace_freer(void *arg) {
  auto *ctx = static_cast<NoreplaceCtx *>(arg);
  ctx->started.fetch_add(1, MemoryOrder::RELEASE);
  while (ctx->started.load(MemoryOrder::ACQUIRE) < 3)
    ::NtYieldExecution();
...
// ---------------------------------------------------------------------------
// 10. Concurrent mmap + mremap + munmap — full lifecycle contention.
//     Exercises mapping table registration, remap guard, section handle
//     duplication, and tombstone generation under concurrent load.
// ---------------------------------------------------------------------------
...
static DWORD __stdcall lifecycle_worker(void *arg) {
