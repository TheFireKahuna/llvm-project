...
static DWORD __stdcall raise_thread(void *) {
...
static DWORD __stdcall sigqueue_producer(void *arg) {
  auto *ctx = static_cast<ProducerConsumerCtx *>(arg);
  for (int i = 0; i < 50; i++) {
    union sigval val;
    val.sival_int = i;
    LIBC_NAMESPACE::sigqueue(getpid(), SIGUSR2, val);
...
static DWORD __stdcall siginfo_raise_thread(void *) {
  for (int i = 0; i < 200; i++)
    LIBC_NAMESPACE::raise(SIGUSR1);
  return 0;
}
