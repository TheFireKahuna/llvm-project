...
static volatile bool inner_handler_ran_during_mask;
...
TEST(LlvmLibcWindowsSigaction, SaMaskBlocksDuringHandler) {
  ...
  inner_handler_ran_during_mask = false;
  ...
  action1.sa_handler = +[](int) {
    // SIGUSR2 should be blocked during this handler; the raise() below
    // queues it as pending rather than nesting into SIGUSR2's handler.
    // We can't use EXPECT_FALSE in a captureless lambda, so record the
    // observation for the outer scope to assert.
    inner_handler_called = false;
    LIBC_NAMESPACE::raise(SIGUSR2);
    inner_handler_ran_during_mask = inner_handler_called;
  };
  LIBC_NAMESPACE::sigaction(SIGUSR1, &action1, nullptr);

  LIBC_NAMESPACE::raise(SIGUSR1);

  // SIGUSR2 must have been blocked during SIGUSR1's handler.
  EXPECT_FALSE(inner_handler_ran_during_mask);

  // The APC-based dispatch path can land SIGUSR2 delivery on the next
  // alertable return rather than synchronously at sigprocmask restore, so
  // poll an alertable wait until the pending SIGUSR2 drains (or we time
  // out and fail the assertion below).
  for (int attempt = 0; attempt < 100 && !inner_handler_called; ++attempt)
