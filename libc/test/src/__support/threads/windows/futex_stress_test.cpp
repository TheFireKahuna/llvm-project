...
static DWORD __stdcall ping_pong_even(void *arg) {
...
static DWORD __stdcall ping_pong_odd(void *arg) {
  auto *ctx = static_cast<PingPongCtx *>(arg);
  for (int i = 0; i < PING_PONG_ROUNDS; i++) {
...
static DWORD __stdcall timeout_waiter(void *arg) {
...
static DWORD __stdcall timeout_waker(void *arg) {
  auto *ctx = static_cast<TimeoutRaceCtx *>(arg);
  while (!ctx->done.load(MemoryOrder::ACQUIRE)) {
...
static DWORD __stdcall notify_all_waiter(void *arg) {
...
static DWORD __stdcall rapid_exit_thread(void *) {
...
TEST(LlvmLibcFutexStressTest, RapidThreadExitSlotRecycle) {
  ...
  // If we get here without crashing, the slot lifecycle is sound.
  // Total slots used: 10 * 32 = 320, well within pool of 4096.
  EXPECT_TRUE(true);
}
...
static DWORD __stdcall multi_futex_worker(void *arg) {
