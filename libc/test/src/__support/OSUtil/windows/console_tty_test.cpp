...
struct PtyPair {
  ...
  PtyPair() {
    if (LIBC_NAMESPACE::openpty(&master, &slave, nullptr, nullptr, nullptr) ==
        0)
      valid = true;
  }
  ...
  ~PtyPair() {
    if (slave >= 0)
      LIBC_NAMESPACE::close(slave);
    if (master >= 0)
      LIBC_NAMESPACE::close(master);
  }
...
bool write_to_master(int master_fd, const char *data, size_t len) {
  ...
  while (remaining > 0) {
    ssize_t n = LIBC_NAMESPACE::write(master_fd, p, remaining);
    if (n <= 0)
      return false;
    p += n;
...
ssize_t read_with_timeout(int fd, char *buf, size_t max) {
  ...
  int old_flags = LIBC_NAMESPACE::fcntl(fd, F_GETFL);
  LIBC_NAMESPACE::fcntl(fd, F_SETFL, old_flags | O_NONBLOCK);

  ssize_t total = 0;
  for (int attempts = 0; attempts < 50; ++attempts) {
    ssize_t n = LIBC_NAMESPACE::read(fd, buf + total,
                                     max - static_cast<size_t>(total));
    if (n > 0) {
      total += n;
      break;
    }
  ...
  LIBC_NAMESPACE::fcntl(fd, F_SETFL, old_flags);
...
void set_raw_mode(int slave_fd, cc_t vmin, cc_t vtime) {
  struct termios t = {};
  LIBC_NAMESPACE::tcgetattr(slave_fd, &t);
  ...
  t.c_cc[VMIN] = vmin;
  t.c_cc[VTIME] = vtime;
  LIBC_NAMESPACE::tcsetattr(slave_fd, TCSANOW, &t);
}
...
TEST(LlvmLibcConsoleTtyTest, CanonicalLineEditVerase) {
  ...
  struct termios t = {};
  LIBC_NAMESPACE::tcgetattr(pty.slave, &t);
...
TEST(LlvmLibcConsoleTtyTest, CanonicalVeofOnEmptyLineReturnsZero) {
  ...
  int flags = LIBC_NAMESPACE::fcntl(pty.slave, F_GETFL);
  LIBC_NAMESPACE::fcntl(pty.slave, F_SETFL, flags | O_NONBLOCK);

  char buf[16] = {};
  // Brief delay to let the PTY process the input.
  LIBC_NAMESPACE::usleep(5000);
  ssize_t n = LIBC_NAMESPACE::read(pty.slave, buf, sizeof(buf));
  // n == 0 means EOF, which is the expected POSIX behavior.
  EXPECT_EQ(n, static_cast<ssize_t>(0));

  LIBC_NAMESPACE::fcntl(pty.slave, F_SETFL, flags);
}
...
TEST(LlvmLibcConsoleTtyTest, NonCanonicalVminZeroVtimeZeroPolling) {
  ...
  LIBC_NAMESPACE::usleep(2000);

  LIBC_NAMESPACE::fcntl(pty.slave, F_SETFL, flags | O_NONBLOCK);
  n = LIBC_NAMESPACE::read(pty.slave, buf, sizeof(buf));
...
TEST(LlvmLibcConsoleTtyTest, OpostOnlcrTranslation) {
  ...
  LIBC_NAMESPACE::tcsetattr(pty.slave, TCSANOW, &t);

  // Write a newline through the slave's output path.
  ASSERT_EQ(LIBC_NAMESPACE::write(pty.slave, "A\nB", 3),
...
TEST(LlvmLibcConsoleTtyTest, OpostOcrnlTranslation) {
  ...
  // OCRNL: \r → \n.
  ASSERT_EQ(LIBC_NAMESPACE::write(pty.slave, "X\rY", 3),
            static_cast<ssize_t>(3));
...
TEST(LlvmLibcConsoleTtyTest, VintrGeneratesSigint) {
  ...
  LIBC_NAMESPACE::sigaction(SIGINT, &sa, &old_sa);
  ...
  LIBC_NAMESPACE::usleep(10000);

  EXPECT_EQ(g_signal_received, SIGINT);

  LIBC_NAMESPACE::sigaction(SIGINT, &old_sa, nullptr);
}
...
TEST(LlvmLibcConsoleTtyTest, VquitGeneratesSigquit) {
  ...
  LIBC_NAMESPACE::sigemptyset(&sa.sa_mask);
  struct sigaction old_sa = {};
  LIBC_NAMESPACE::sigaction(SIGQUIT, &sa, &old_sa);
  ...
  LIBC_NAMESPACE::sigaction(SIGQUIT, &old_sa, nullptr);
}
...
TEST(LlvmLibcConsoleTtyTest, VsuspGeneratesSigtstp) {
  ...
  LIBC_NAMESPACE::sigemptyset(&sa.sa_mask);
  struct sigaction old_sa = {};
  LIBC_NAMESPACE::sigaction(SIGTSTP, &sa, &old_sa);
  ...
  LIBC_NAMESPACE::sigaction(SIGTSTP, &old_sa, nullptr);
}
...
TEST(LlvmLibcConsoleTtyTest, TcgetpgrpFailsOnNonTerminal) {
  ...
  pid_t fg = LIBC_NAMESPACE::tcgetpgrp(pipefd[0]);
...
TEST(LlvmLibcConsoleTtyTest, TcflowStopsAndResumesOutput) {
  ...
  int rc = LIBC_NAMESPACE::tcflow(pty.slave, TCOOFF);
  EXPECT_EQ(rc, 0);

  // tcflow(TCOON) should succeed — resumes output.
  rc = LIBC_NAMESPACE::tcflow(pty.slave, TCOON);
  EXPECT_EQ(rc, 0);
}
...
TEST(LlvmLibcConsoleTtyTest, TcflowInvalidActionFails) {
  ...
  int rc = LIBC_NAMESPACE::tcflow(pty.slave, 99);
  EXPECT_EQ(rc, -1);
...
TEST(LlvmLibcConsoleTtyTest, TcdrainSucceeds) {
  ...
  LIBC_NAMESPACE::write(pty.slave, "test", 4);
  int rc = LIBC_NAMESPACE::tcdrain(pty.slave);
  EXPECT_EQ(rc, 0);
}
...
TEST(LlvmLibcConsoleTtyTest, WinsizeRoundTrips) {
  ...
  ws_set.ws_xpixel = 0;
  ws_set.ws_ypixel = 0;
  int rc = LIBC_NAMESPACE::ioctl(pty.slave, TIOCSWINSZ, &ws_set);
  ASSERT_EQ(rc, 0);

  struct winsize ws_get = {};
  rc = LIBC_NAMESPACE::ioctl(pty.slave, TIOCGWINSZ, &ws_get);
  ASSERT_EQ(rc, 0);
...
TEST(LlvmLibcConsoleTtyTest, WinsizeZeroDimensionAllowed) {
  ...
  ws.ws_row = 0;
  ws.ws_col = 0;
  int rc = LIBC_NAMESPACE::ioctl(pty.slave, TIOCSWINSZ, &ws);
  EXPECT_EQ(rc, 0);
}
...
TEST(LlvmLibcConsoleTtyTest, WinsizeChangeDeliversSigwinch) {
  ...
  LIBC_NAMESPACE::sigemptyset(&sa.sa_mask);
  struct sigaction old_sa = {};
  LIBC_NAMESPACE::sigaction(SIGWINCH, &sa, &old_sa);

  // Set an initial size.
  struct winsize ws = {};
  ws.ws_row = 24;
  ws.ws_col = 80;
  LIBC_NAMESPACE::ioctl(pty.slave, TIOCSWINSZ, &ws);
  LIBC_NAMESPACE::usleep(5000);
  ...
  LIBC_NAMESPACE::sigaction(SIGWINCH, &old_sa, nullptr);
}
...
TEST(LlvmLibcConsoleTtyTest, TcgetattrTcsetattrRoundTrip) {
  ...
  struct termios orig = {};
  ASSERT_EQ(LIBC_NAMESPACE::tcgetattr(pty.slave, &orig), 0);

  // Flip ECHO.
  struct termios modified = orig;
  modified.c_lflag ^= ECHO;
  ASSERT_EQ(LIBC_NAMESPACE::tcsetattr(pty.slave, TCSANOW, &modified), 0);

  struct termios readback = {};
  ASSERT_EQ(LIBC_NAMESPACE::tcgetattr(pty.slave, &readback), 0);
  EXPECT_EQ(readback.c_lflag & ECHO, modified.c_lflag & ECHO);

  // Restore.
  LIBC_NAMESPACE::tcsetattr(pty.slave, TCSANOW, &orig);
}
...
TEST(LlvmLibcConsoleTtyTest, TcflushInput) {
  ...
  ASSERT_EQ(LIBC_NAMESPACE::tcflush(pty.slave, TCIFLUSH), 0);
...
TEST(LlvmLibcConsoleTtyTest, IsattyOnPtySlave) {
  ...
  EXPECT_EQ(LIBC_NAMESPACE::isatty(pty.slave), 1);
  EXPECT_EQ(LIBC_NAMESPACE::isatty(pty.master), 1);
}

TEST(LlvmLibcConsoleTtyTest, IsattyOnPipeFails) {
  int pipefd[2];
  ASSERT_EQ(LIBC_NAMESPACE::pipe(pipefd), 0);
  EXPECT_NE(LIBC_NAMESPACE::isatty(pipefd[0]), 1);
  EXPECT_NE(LIBC_NAMESPACE::isatty(pipefd[1]), 1);
  LIBC_NAMESPACE::close(pipefd[0]);
  LIBC_NAMESPACE::close(pipefd[1]);
}
...
TEST(LlvmLibcConsoleTtyTest, FionreadReturnsZeroOnEmptyQueue) {
  ...
  int count = -1;
  int rc = LIBC_NAMESPACE::ioctl(pty.slave, FIONREAD, &count);
  ASSERT_EQ(rc, 0);
  EXPECT_EQ(count, 0);
}
...
TEST(LlvmLibcConsoleTtyTest, TcsendbreakSucceeds) {
  ...
  int rc = LIBC_NAMESPACE::tcsendbreak(pty.slave, 0);
  EXPECT_EQ(rc, 0);
}
...
TEST(LlvmLibcConsoleTtyTest, TcsendbreakFailsOnNonTerminal) {
  ...
  int rc = LIBC_NAMESPACE::tcsendbreak(pipefd[0], 0);
  EXPECT_EQ(rc, -1);
  EXPECT_EQ(errno, ENOTTY);
