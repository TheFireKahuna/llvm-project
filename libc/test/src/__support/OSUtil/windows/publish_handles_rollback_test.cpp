...
TEST(LlvmLibcPublishHandlesTest, PublishAndRestorePreservesPebState) {
  ...
  HANDLE console = condrv::get_console_handle();
  ...
  NTSTATUS status = console::launch(&session, opts);
  if (!NT_SUCCESS(status))
    return; // Can't launch console — skip gracefully.

  // publish_handles with saved state.
  console::PublishedState saved = {};
  status = console::publish_handles(session.handles,
  ...
  status = console::restore_handles(&saved);
...
TEST(LlvmLibcPublishHandlesTest, PublishWithNullConnectionFails) {
  ...
  condrv::CONDRV_CLIENT_HANDLES bad_handles = {};
  bad_handles.Connection = nullptr; // invalid

  console::PublishedState saved = {};
  NTSTATUS status =
      console::publish_handles(bad_handles, nullptr, &saved);
