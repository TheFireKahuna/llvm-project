...
TEST(LlvmLibcPtyLifecycleTest, FullAllocatePreparePtsnameOpenLifecycle) {
  ...
  char slave_name[64] = {};
  ASSERT_EQ(LIBC_NAMESPACE::ptsname_r(ptmx_fd, slave_name, sizeof(slave_name)),
            0);
  ASSERT_GT(LIBC_NAMESPACE::strlen(slave_name), static_cast<size_t>(0));
  ...
  EXPECT_EQ(LIBC_NAMESPACE::isatty(slave_fd), 1);

  // Verify terminal attributes are readable.
  struct termios attrs = {};
  EXPECT_EQ(LIBC_NAMESPACE::tcgetattr(slave_fd, &attrs), 0);

  // Verify window size is queryable.
  struct winsize ws = {};
  EXPECT_EQ(LIBC_NAMESPACE::ioctl(slave_fd, TIOCGWINSZ, &ws), 0);
...
TEST(LlvmLibcPtyLifecycleTest, MasterSlaveIOPassthrough) {
  ...
  ASSERT_EQ(
      LIBC_NAMESPACE::openpty(&master_fd, &slave_fd, nullptr, nullptr, nullptr),
      0);
  ...
  size_t msg_len = LIBC_NAMESPACE::strlen(msg);
  ASSERT_EQ(LIBC_NAMESPACE::write(master_fd, msg, msg_len),
            static_cast<ssize_t>(msg_len));

  char buf[64] = {};
  // Set non-blocking with a small retry to handle async delivery.
  int flags = LIBC_NAMESPACE::fcntl(slave_fd, F_GETFL);
  LIBC_NAMESPACE::fcntl(slave_fd, F_SETFL, flags | O_NONBLOCK);

  ssize_t total = 0;
  for (int i = 0; i < 50 && total < static_cast<ssize_t>(msg_len); ++i) {
    ssize_t n = LIBC_NAMESPACE::read(slave_fd, buf + total,
  ...
  LIBC_NAMESPACE::fcntl(slave_fd, F_SETFL, flags);

  ASSERT_EQ(total, static_cast<ssize_t>(msg_len));
  EXPECT_EQ(LIBC_NAMESPACE::memcmp(buf, msg, msg_len), 0);

  // Slave → Master: write to slave (raw, no OPOST), read from master.
  const char *reply = "reply from slave";
  size_t reply_len = LIBC_NAMESPACE::strlen(reply);
  ASSERT_EQ(LIBC_NAMESPACE::write(slave_fd, reply, reply_len),
            static_cast<ssize_t>(reply_len));

  char rbuf[64] = {};
  flags = LIBC_NAMESPACE::fcntl(master_fd, F_GETFL);
  LIBC_NAMESPACE::fcntl(master_fd, F_SETFL, flags | O_NONBLOCK);

  total = 0;
  for (int i = 0; i < 50 && total < static_cast<ssize_t>(reply_len); ++i) {
    ssize_t n = LIBC_NAMESPACE::read(master_fd, rbuf + total,
  ...
  LIBC_NAMESPACE::fcntl(master_fd, F_SETFL, flags);

  ASSERT_EQ(total, static_cast<ssize_t>(reply_len));
  EXPECT_EQ(LIBC_NAMESPACE::memcmp(rbuf, reply, reply_len), 0);

  LIBC_NAMESPACE::close(slave_fd);
  LIBC_NAMESPACE::close(master_fd);
}
...
TEST(LlvmLibcPtyLifecycleTest, OpenptyPreservesTermiosAndWinsize) {
  ...
  int master_fd = -1, slave_fd = -1;
  int rc = LIBC_NAMESPACE::openpty(&master_fd, &slave_fd, nullptr,
                                   &custom_attrs, &custom_ws);
  ASSERT_EQ(rc, 0);

  // Verify the termios were applied.
  struct termios actual_attrs = {};
  ASSERT_EQ(LIBC_NAMESPACE::tcgetattr(slave_fd, &actual_attrs), 0);
  ...
  ASSERT_EQ(LIBC_NAMESPACE::ioctl(slave_fd, TIOCGWINSZ, &actual_ws), 0);
...
TEST(LlvmLibcPtyLifecycleTest, ChildProcessCanOpenAndUseSlave) {
  ...
  ASSERT_EQ(LIBC_NAMESPACE::posix_spawn_file_actions_adddup2(
                &actions, pipe_fds[0], CHILD_CONTROL_FD),
            0);
  ...
  int spawn_rc = LIBC_NAMESPACE::posix_spawn(
      &child_pid, child_path, &actions, nullptr, child_argv, environ);
...
TEST(LlvmLibcPtyLifecycleTest, MultipleSlavesFromSameMaster) {
  ...
  int slave1 = LIBC_NAMESPACE::open(slave_name, O_RDWR | O_NOCTTY | O_CLOEXEC);
  ASSERT_GE(slave1, 0);

  int slave2 = LIBC_NAMESPACE::open(slave_name, O_RDWR | O_NOCTTY | O_CLOEXEC);
  ASSERT_GE(slave2, 0);

  EXPECT_NE(slave1, slave2);
  EXPECT_EQ(LIBC_NAMESPACE::isatty(slave1), 1);
  EXPECT_EQ(LIBC_NAMESPACE::isatty(slave2), 1);

  LIBC_NAMESPACE::close(slave2);
  LIBC_NAMESPACE::close(slave1);
  LIBC_NAMESPACE::close(ptmx_fd);
}
...
TEST(LlvmLibcPtyLifecycleTest, ClosingMasterSignalsSlaveEof) {
  ...
  char buf[16] = {};
  ssize_t n = LIBC_NAMESPACE::read(slave_fd, buf, sizeof(buf));
