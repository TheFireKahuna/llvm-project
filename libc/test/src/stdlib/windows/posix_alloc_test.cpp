...
#include "src/stdlib/windows/posix_alloc_size_classes.h"
...
// ---------------------------------------------------------------------------
// Size class coverage — allocates one object from every production slab class.
// Pulls the class list from the shared header so the test cannot silently
// diverge from the runtime table if a new class is added or removed.
// ---------------------------------------------------------------------------

TEST(LlvmLibcPosixAllocTest, AllSizeClasses) {
  void *ptrs[LIBC_NAMESPACE::POSIX_ALLOC_NUM_CLASSES];
  for (int i = 0; i < LIBC_NAMESPACE::POSIX_ALLOC_NUM_CLASSES; i++) {
    uint16_t sz = LIBC_NAMESPACE::POSIX_ALLOC_SIZE_CLASSES[i];
    ptrs[i] = LIBC_NAMESPACE::posix_alloc(sz);
    ASSERT_NE(ptrs[i], static_cast<void *>(nullptr));
    LIBC_NAMESPACE::inline_memset(ptrs[i], 0xAB, sz);
  }
  for (int i = 0; i < LIBC_NAMESPACE::POSIX_ALLOC_NUM_CLASSES; i++)
...
static DWORD xthread_free_worker(void *arg) {
...
static DWORD stress_worker(void *arg) {
