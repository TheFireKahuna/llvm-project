...
TEST(LlvmLibcInetPton, ParsesStrictIpv4Address) {
  uint8_t out[4] = {};
...
TEST(LlvmLibcInetPton, RejectsLegacyIpv4Notation) {
  uint8_t out[4] = {};
  ASSERT_EQ(0, inet_pton(AF_INET, "1.2.03.4", out));
  ASSERT_EQ(0, inet_pton(AF_INET, "1.2.0xabcd", out));
}

TEST(LlvmLibcInetPton, ParsesIpv6Addresses) {
  uint8_t out[16] = {};
...
TEST(LlvmLibcInetPton, UnsupportedFamilyFails) {
  uint8_t out[16] = {};
  libc_errno = 0;
