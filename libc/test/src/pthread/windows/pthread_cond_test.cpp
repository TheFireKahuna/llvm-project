...
// pthread_cond_signal wakes one waiter; the other stays blocked until
// a second signal.
TEST(LlvmLibcWindowsPthreadCondTest, SignalWakesOne) {
  ...
  // Signal twice — both threads should wake.
  // The threads will spin inside cond_wait until woken; signalling twice
  // (once per waiter) is correct regardless of scheduling order.
