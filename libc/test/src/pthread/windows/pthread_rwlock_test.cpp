...
TEST(LlvmLibcWindowsPthreadRwlockTest, ConcurrentReaders) {
  ...
  // a serialising implementation would observe max_concurrent == 1 and
  // silently pass the weaker bound).
  EXPECT_GE(s.max_concurrent.load(LIBC_NAMESPACE::cpp::MemoryOrder::ACQUIRE), 2);
