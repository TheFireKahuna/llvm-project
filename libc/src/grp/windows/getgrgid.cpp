//===-- Windows implementation of getgrgid --------------------------------===//
//
...
// POSIX getgrgid: returns nullptr without changing errno on not-found.
LLVM_LIBC_FUNCTION(struct group *, getgrgid, (gid_t gid)) {
  static thread_local struct group grp;
  static thread_local char buf[1024];

  int err = internal::fill_group_gid(gid, &grp, buf, sizeof(buf));
  if (err == ENOENT)
    return nullptr;
  if (err != 0) {
    libc_errno = err;
