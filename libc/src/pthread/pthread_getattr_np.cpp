...
LLVM_LIBC_FUNCTION(int, pthread_getattr_np,
  ...
  if (is_self || !th) {
    ...
    // Scheduling: use the calling thread's own attrib (sched_support
    // detects self via attrib->tid == NtCurrentThreadId()).
    ThreadAttributes *self_attrib = self.attrib;
    int err = self_attrib ? sched_support::get_thread_sched(
                                self_attrib, &attr->__schedpolicy,
                                &attr->__schedparam)
                          : ESRCH;
    if (err) {
    ...
    // --- Cross-thread query ---
    // pthread_t IS the inline ThreadAttributes inside a registered
    // ThreadLifecycle (ThreadLifecycle::attrib). Match the caller's
    // pointer against `&lc->attrib` — the offset-aliased identity —
    // and use the validated lifecycle's metadata to fill the attr.
    ...
    registry_for_each([&](ThreadLifecycle *lc) -> bool {
      ...
      found = true;
      fill_attr_from_lifecycle(attr, lc);
      int err = sched_support::get_thread_sched(
          &lc->attrib, &attr->__schedpolicy, &attr->__schedparam);
      if (err) {
        attr->__schedpolicy = SCHED_OTHER;
        attr->__schedparam.sched_priority = 0;
      }
