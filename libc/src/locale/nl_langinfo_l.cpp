...
#ifdef LIBC_TARGET_OS_IS_WINDOWS
  // LC_GLOBAL_LOCALE means "use the current global/thread locale" — fall
  // through to nl_langinfo() which already reads it. Without this guard the
  // sentinel ((locale_t)-1) would be dereferenced as a real locale_t.
  if (loc && loc != LC_GLOBAL_LOCALE && loc != &c_locale) {
