...
static LIBC_THREAD_LOCAL internal::SlabPool::ThreadSlab locale_slab = nullptr;
...
static LocaleAlloc *alloc_locale() {
  ...
  void *slot = internal::SlabPool::alloc(locale_slab);
  if (!slot)
    slot = locale_pool.alloc_slow(&locale_slab);
...
LLVM_LIBC_FUNCTION(locale_t, newlocale,
  ...
  // Look up the NLS record for this locale name.
