...
static char *c_langinfo(nl_item item) {
  switch (item) {
  case CODESET:
    // NT-POSIX is a UTF-8-only locale profile (see locale.h): setlocale /
    // newlocale reject any explicit non-UTF-8 codeset suffix, so the C
    // locale is the only locale that could plausibly disagree. Aligning
    // it with the rest of the profile (and with upstream llvm-libc / musl
    // / glibc C.UTF-8) keeps the libc internally consistent: every
    // multibyte conversion path assumes UTF-8 unconditionally, and the
    // CODESET advertisement matches that assumption rather than lying
    // about a single-byte encoding the converter never implemented.
