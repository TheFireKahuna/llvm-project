//===-- Windows implementation of sigsuspend -------------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Thin entry point — delegates to internal::sigsuspend() in
// signal_syscalls.cpp.
//
//===----------------------------------------------------------------------===//
...
#include "src/__support/OSUtil/windows/syscall_wrappers/sigsuspend.h"
...
LLVM_LIBC_FUNCTION(int, sigsuspend, (const sigset_t *mask)) {
  ...
  auto result = windows_syscalls::sigsuspend(mask);
  if (!result.has_value()) {
