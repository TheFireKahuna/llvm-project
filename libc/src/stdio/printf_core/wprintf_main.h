...
wprintf_main(WideWriterT *writer, const wchar_t *__restrict str,
  ...
  for (WideFormatSection cur_section = parser.get_next_section();
       !cur_section.raw_string.empty() || cur_section.has_conv;
       cur_section = parser.get_next_section()) {
    if (cur_section.has_conv) {
      result = wide_convert(writer, cur_section);
    } else {
      // Literal text — write wide characters directly.
      result = writer->write_wide(cur_section.raw_string.data(),
                                  cur_section.raw_string.size());
    }
    if (result < 0)
      return Error(-result);
  }
