...
// Run a narrow converter and widen its output into the wide writer. Takes the
// shared char-agnostic descriptor by const-ref; the wide caller implicitly
// upcasts its basic_format_section<wchar_t> to the FormatDesc base — no field
// copy, no drift surface. The narrow converter's default (unknown-conv) arm
// echoes raw_string, but wide_convert services unknown conversions upstream,
// so the narrow bridge never needs raw_string.
template <typename WideWriterT>
LIBC_INLINE int narrow_to_wide(WideWriterT *wwriter, const FormatDesc &desc) {
  ...
  FormatSection narrow_bridge;
  static_cast<FormatDesc &>(narrow_bridge) = desc; // slice-copy the descriptor
  int result = convert(&writer, narrow_bridge);
  if (result != WRITE_OK)
    return result;
