...
// Test whether cur_char is in the scanset for %[ conversions.
template <typename CharType>
LIBC_INLINE bool in_scanset(const FormatSection &to_conv, CharType cur_char) {
  if constexpr (cpp::is_same_v<CharType, char>)
    return to_conv.scan_set.test(static_cast<unsigned char>(cur_char));
  else
    return to_conv.wide_scan_set.test(cur_char);
}

// Same-width string conversion: CharType in, CharType out.
template <typename T, typename CharType>
int convert_string_same(Reader<T, CharType> *reader,
                        const FormatSection &to_conv, size_t max_width) {
  ...
  for (; i < max_width && cur_char != CharType(); ++i) {
    if ((to_conv.conv_name == 's' && internal::isspace(cur_char)) ||
        (to_conv.conv_name == '[' && !in_scanset(to_conv, cur_char))) {
      break;
    }
...
template <typename T>
int convert_string_wide_to_narrow(Reader<T, wchar_t> *reader,
...
template <typename T>
int convert_string_narrow_to_wide(Reader<T, char> *reader,
                                  const FormatSection &to_conv,
                                  size_t max_width) {
  ...
  for (; wchars_written < max_width && cur_char != '\0';) {
    ...
    internal::CharacterConverter cr(&state, internal::locale_encoding_is_utf8());
    while (cur_char != '\0') {
      int err = cr.push(static_cast<char8_t>(static_cast<unsigned char>(cur_char)));
      if (err != 0)
        return MATCHING_FAILURE;
      if (cr.isFull())
        break;
    ...
    if ((to_conv.conv_name == 's' && internal::isspace(wc)) ||
        (to_conv.conv_name == '[' && !in_scanset(to_conv, wc))) {
      reader->ungetc(cur_char);
      break;
    }
    if ((to_conv.flags & NO_WRITE) == 0)
...
int convert_string(Reader<T, CharType> *reader, const FormatSection &to_conv) {
  size_t max_width = 0;
