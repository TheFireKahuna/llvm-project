//===-- Standalone implementation of std::basic_string_view ----*- C++ -*-===//
//
...
// This is a very simple alternate of std::basic_string_view. There is no
// bounds check performed in any of the methods. The callers are expected to
// do the checks before invoking the methods.
//
// This class will be extended as needed in future. `wchar_t` and `char16_t`
// are C++ built-in types, so no extra headers are required to use the
// `wstring_view` / `w16string_view` aliases.
//
// Naming note: `w16string_view` is an ABI-width view over `char16_t`. It is
// distinct from `std::u16string_view` in intent — the `u16` name in the
// standard implies a UTF-16 *encoding*, whereas our `w16` alias promises
// only a fixed-width code-unit view. The NT WCHAR block, for example, is a
// 16-bit code-unit stream that can legally contain lone surrogates, so it is
// naturally a `w16string_view` rather than a `u16string_view`. It is also
// distinct from `wstring_view`, which tracks the platform `wchar_t` width
// (16 bits on Windows Itanium, 32 bits on NT-POSIX under `-fwchar-type=int`)
// and is reserved for the POSIX wide-I/O surface (`fputwc` etc.).
...
private:
  ...
  compareMemory(const CharT *Lhs, const CharT *Rhs, size_t Length) {
...
using w16string_view = basic_string_view<char16_t>;
