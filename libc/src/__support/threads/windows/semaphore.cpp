...
int Semaphore::wait() {
  for (;;) {
    ...
    //
    // Fatal-error exit: a negative return other than -EINTR (the
    // default Futex::wait<false> absorbs APCs internally, so -EINTR
    // shouldn't surface) means we cannot park — -ENOMEM from wait-
    // slot pool exhaustion being the canonical case. Re-looping would
    // hit val==0 again and re-fail identically. Surface the error so
    // the caller doesn't spin on an unacquirable sem_t.
    long ret = count.wait(0);
    if (ret < 0 && ret != -EINTR)
...
int Semaphore::timedwait(const internal::AbsTimeout &timeout) {
  for (;;) {
    ...
    // Fatal-error exit: any other negative return (e.g. -ENOMEM on
    // wait-slot pool exhaustion, -EINVAL on sem_t destruction) means
    // the wait cannot make progress. Surface the errno rather than
    // spin-failing in the enclosing for(;;).
    if (ret < 0 && ret != -EINTR)
      return static_cast<int>(-ret);
  }
}
