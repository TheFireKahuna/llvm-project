...
// alignas(16) keeps sizeof(TimerSlot) a multiple of 16 so that
// SLOTS_PER_CHUNK (256) × sizeof(TimerSlot) stays 4 KB-aligned — the
// invariant IndexedPool::kChunkLayout::is_valid() enforces via
// static_assert. Without the alignment, adding a small field to
// TimerSlot silently breaks chunk geometry.
struct alignas(16) TimerSlot {
  HANDLE nt_timer;
...
inline void timer_reactor_cb(void *context, NTSTATUS /*status*/,
  ...
  case SIGEV_THREAD: {
    ...
    windows::ScopedNtHandle th;
    SIZE_T stack_reserve = slot->thread_stack_size;
    ::NtCreateThreadEx(
        th.put(), SYNCHRONIZE, nullptr, NtCurrentProcess(),
        reinterpret_cast<PVOID>(sigev_thread_entry), slot, 0, 0, 0,
        stack_reserve, nullptr);
    // Detached — we don't join SIGEV_THREAD threads. th closes on scope exit.
    break;
  }
  case SIGEV_NONE:
  default:
