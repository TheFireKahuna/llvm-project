...
// Set the DISABLE bit in cancel_word. Prevents re-entry if a destructor
// or cleanup handler calls a cancellation point during unwind.
static void disable_cancel() {
  auto *word = cancel::get_cancel_word();
  if (!word)
    return;
  uint8_t old = word->load(cpp::MemoryOrder::RELAXED);
  uint8_t desired;
  do {
    desired = old | cancel::STATE_BIT;
  } while (!word->compare_exchange_weak(old, desired,
