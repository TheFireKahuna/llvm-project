...
ErrorOr<int> platform_opendir(const char *name) {
  ...
  windows::nt_wstring_view nt_name(path_buf, path_len);
  OBJECT_ATTRIBUTES oa;
  init_object_attributes(&oa, &nt_name);
...
static unsigned char probe_fifo_marker(HANDLE parent_dir,
  ...
  windows::nt_wstring_view entry_wsv(name, name_len_bytes / sizeof(WCHAR));

  OBJECT_ATTRIBUTES oa;
  oa.Length = sizeof(oa);
  oa.RootDirectory = parent_dir;
  oa.ObjectName = entry_wsv.unicode_string();
  oa.Attributes = 0;
...
pack_dirent(uint8_t *out, size_t avail,
  ...
  char name_buf[1024];
  size_t wchar_count = nt_entry->FileNameLength / sizeof(WCHAR);
  int utf8_result = LIBC_NAMESPACE::windows::utf16_to_utf8(
      nt_entry->FileName, wchar_count, name_buf, sizeof(name_buf) - 1);
  if (utf8_result <= 0)
    return DIRENT_SKIP;
  name_buf[utf8_result] = '\0';

  size_t name_len = static_cast<size_t>(utf8_result);
