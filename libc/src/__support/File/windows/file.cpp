...
//===----------------------------------------------------------------------===//
// Stdio -> ofd->ops dispatch shims
//===----------------------------------------------------------------------===//

// Stdio writes delegate to the per-kind ofd->ops->write. Each FileKind has
// its own impl in io/read_write.cpp:
//   Disk   -> IoRing with O_APPEND/O_DIRECT/rlimit handling + BatchEngine
//   Pipe   -> IoRing
//   Char   -> ioring_simple_write (NtWriteFile on sync handle)
//   ConDrv -> console-driver message path
//   Pipe-backed AFD/Fifo/SocketPair/Pty -> ring-buffer channels
//   Inotify/Epoll/VirtualDir -> kind-specific error (EINVAL/EISDIR)
// Dispatching here instead of reimplementing NtWriteFile means stdio
// writes (fwrite, printf, fputs) get the same fast path as internal::write
// and inherit every per-kind behavior — including SIGPIPE generation,
// SA_RESTART, O_NONBLOCK EAGAIN, and dup2 redirection semantics.
FileIOResult WindowsFile::platform_write(File *f, const void *data, size_t size) {
  ...
  if (!ofd || !ofd->ops || !ofd->ops->write)
    return {0, EBADF};

  ssize_t r = ofd->ops->write(ofd, data, size);
  if (r < 0)
    return {0, static_cast<int>(-r)};
  return static_cast<size_t>(r);
}

// See platform_write for rationale — delegates to ofd->ops->read for the
// per-kind fast path (IoRing for Disk/Pipe, channels for Pty/Sp/Fifo,
// ConDrv-specific for the console).
FileIOResult WindowsFile::platform_read(File *f, void *buf, size_t size) {
  auto *wf = static_cast<WindowsFile *>(f);
  auto *ofd = internal::fd_table.get_ofd(wf->get_fd());
  if (!ofd || !ofd->ops || !ofd->ops->read)
    return {0, EBADF};

  ssize_t r = ofd->ops->read(ofd, buf, size);
  if (r < 0)
    return {0, static_cast<int>(-r)};
  return static_cast<size_t>(r);
}
...
FileIOResult IoRingFile::platform_write(File *f, const void *data, size_t size) {
  ...
  if (!evt)
    return {0, EIO};
  result = ioring_wait_cqe(tr, irf->get_handle(), gen);
  if (result.has_error()) {
    if (result.error == EPIPE)
      raise_sigpipe_if_needed();
    return result;
...
FileIOResult IoRingFile::platform_read(File *f, void *buf, size_t size) {
  ...
  FileIOResult result = ioring_submit_and_wait(tr, irf->get_handle(), gen);
  if (!result.has_error()) {
    if (result.value == 0)
      return {0, 0}; // EOF
    irf->advance_position(static_cast<int64_t>(result.value));
...
ErrorOr<File *> openfile(const char *path, const char *mode) {
  ...
  auto nt = to_nt_path(sv, path_buf, MAX_NT_PATH_WCHARS);
  if (!nt.has_value())
...
// `.libcfio$M` ops tables consumed by the WindowsFile / IoRingFile
// ctors via kind_to_windows_file_ops(). Defined here so that linking
// this TU is the only way for a WindowsFileKind to populate its slot --
// a build that drops file.cpp drops the corresponding registration and
// the ctor's lookup traps on null, surfacing the missing dependency
// immediately instead of at first I/O.
const WindowsFileOps windows_file_sync_ops = {
    &::LIBC_NAMESPACE::WindowsFile::platform_write,
    &::LIBC_NAMESPACE::WindowsFile::platform_read,
    &::LIBC_NAMESPACE::WindowsFile::platform_seek,
    &::LIBC_NAMESPACE::WindowsFile::platform_close,
    /*sync=*/nullptr,
};

const WindowsFileOps windows_file_ioring_ops = {
    &::LIBC_NAMESPACE::IoRingFile::platform_write,
    &::LIBC_NAMESPACE::IoRingFile::platform_read,
    &::LIBC_NAMESPACE::IoRingFile::platform_seek,
    &::LIBC_NAMESPACE::IoRingFile::platform_close,
    &::LIBC_NAMESPACE::IoRingFile::platform_sync,
};
...
// `.libcfio$M` registrations. Must expand at namespace scope (outside
// any namespace {} block) -- LIBC_SECTION_REGISTER opens its own
// `LIBC_NAMESPACE_DECL::internal` block.
LIBC_REGISTER_WINDOWS_FILE_OPS(Sync,
                               ::LIBC_NAMESPACE::internal::windows_file_sync_ops)
LIBC_REGISTER_WINDOWS_FILE_OPS(
    IoRing, ::LIBC_NAMESPACE::internal::windows_file_ioring_ops)
