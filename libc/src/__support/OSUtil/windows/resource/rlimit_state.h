...
inline bool ensure_self_job() {
  ...
  auto oa = internal_oa();
  NTSTATUS st =
      ::NtCreateJobObject(&job, JOB_OBJECT_ALL_ACCESS, &oa);
  if (!NT_SUCCESS(st))
    return false;

  st = ::NtAssignProcessToJobObject(job, NtCurrentProcess());
  if (!NT_SUCCESS(st)) {
    ::NtClose(job);
    return false;
  }

  g_pcb.rlimit.job = job;
  return true;
}
...
inline NTSTATUS apply_job_limits() {
  ...
  // RLIMIT_CORE -> suppress WER crash dialogs and dumps when set to 0.
  if (lim[RLIMIT_CORE].rlim_cur == 0) {
    flags |= JOB_OBJECT_LIMIT_DIE_ON_UNHANDLED_EXCEPTION;
    // Also suppress the GPF error dialog for this process via NT API.
    ULONG error_mode = SEM_NOGPFAULTERRORBOX;
