...
uid_t query_process_uid(HANDLE process) {
  ...
  ULONG needed = 0;
  st = ::NtQueryInformationToken(token, TokenUser, buf, sizeof(buf), &needed);
  ::NtClose(token);
  if (!NT_SUCCESS(st))
    return static_cast<uid_t>(-1);
...
template <typename Fn> void for_each_process(ACCESS_MASK access, Fn &&visitor) {
  ...
  for (;;) {
    HANDLE next = nullptr;
    NTSTATUS st = ::NtGetNextProcess(prev, access, 0, 0, &next);
    if (prev)
      ::NtClose(prev);
...
intptr_t getpriority_process(id_t who) {
  ...
  HANDLE proc = nullptr;
  NTSTATUS st =
      NtOpenProcessById(&proc, PROCESS_QUERY_LIMITED_INFORMATION,
...
intptr_t setpriority_process(id_t who, int nice) {
  ...
  HANDLE proc = nullptr;
  NTSTATUS st =
      NtOpenProcessById(&proc, PROCESS_SET_INFORMATION,
                        static_cast<DWORD>(who));
  if (!NT_SUCCESS(st))
    return -ESRCH;
  int err = set_process_nice(proc, nice);
  ::NtClose(proc);
  return err;
}
...
intptr_t setpriority_pgrp(id_t who, int nice) {
  ...
  for_each_process(access, [&](HANDLE proc) -> bool {
    ...
    if (pid == self_pid) {
      ...
      HANDLE set_proc = nullptr;
      NTSTATUS st = NtOpenProcessById(&set_proc, PROCESS_SET_INFORMATION,
                                      static_cast<DWORD>(pid));
      if (NT_SUCCESS(st)) {
        int err = set_process_nice(set_proc, nice);
        ::NtClose(set_proc);
        if (err != 0)
          last_err = err;
      } else {
