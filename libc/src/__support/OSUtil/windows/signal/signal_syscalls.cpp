...
static bool pend_signal(PendingSet &ps, int signum, int si_code) {
  if (signum >= SIGRTMIN) {
    ...
    signal_pending::pend_standard(ps, signum);
  }
  return true;
}
...
// Signals delivered via VEH (hardware exceptions).
static bool is_seh_signal(int signum) {
  switch (signum) {
  case SIGSEGV:
  case SIGBUS:
  case SIGFPE:
...
// ---------------------------------------------------------------------------
intptr_t rt_sigaction(int signum, const void *act_arg, void *oldact_arg) {
  auto *act = static_cast<const struct sigaction *>(act_arg);
  auto *oldact = static_cast<struct sigaction *>(oldact_arg);
  ...
  // Lazily ensure the appropriate Windows mechanism is registered before
  // the handler becomes visible.
  if (act && act->sa_handler != SIG_DFL && act->sa_handler != SIG_IGN) {
    if (is_seh_signal(signum))
      veh_transport::install();
    else if (is_console_signal(signum))
  ...
  if (act && signum == SIGINT)
    console::set_ctrl_c_ignore(act->sa_handler == SIG_IGN);
...
// ---------------------------------------------------------------------------
intptr_t rt_sigprocmask(int how, const void *set_arg, void *oldset_arg) {
  auto *set = static_cast<const sigset_t *>(set_arg);
  auto *oldset = static_cast<sigset_t *>(oldset_arg);
  ...
  // Keep the console handler's blocked-mask snapshot in sync.
  auto &dispatch = g_pcb.signal_dispatch;
  auto *lc = get_current_lifecycle();
  if (lc &&
      dispatch.preferred_thread.load(cpp::MemoryOrder::RELAXED) == lc)
...
static int terminate_process(DWORD pid, int signum) {
  HANDLE h = nullptr;
  NTSTATUS st = ::NtOpenProcessById(&h, PROCESS_TERMINATE, pid);
  if (!NT_SUCCESS(st))
    return (st == STATUS_ACCESS_DENIED) ? EPERM : ESRCH;
  st = ::NtTerminateProcess(h, 128 + signum);
  ::NtClose(h);
  if (!NT_SUCCESS(st))
    return EPERM;
  return 0;
...
static int suspend_process(DWORD pid) {
  HANDLE h = nullptr;
  NTSTATUS st = ::NtOpenProcessById(&h, PROCESS_SUSPEND_RESUME, pid);
  if (!NT_SUCCESS(st))
    return (st == STATUS_ACCESS_DENIED) ? EPERM : ESRCH;
  HANDLE sc = nullptr;
  auto sc_oa = windows::internal_oa();
  st = ::NtCreateProcessStateChange(&sc, PROCESS_STATE_ALL_ACCESS, &sc_oa, h,
                                    0);
  if (!NT_SUCCESS(st)) {
    ::NtClose(h);
    return EPERM;
  }
  st = ::NtChangeProcessState(sc, h, ProcessStateSuspend, nullptr, 0, 0);
  ::NtClose(h);
  if (!NT_SUCCESS(st)) {
    ::NtClose(sc);
    return EPERM;
  }
  // sc kept open — closing would auto-resume.
  return 0;
}
...
static int deliver_signal_to_process(DWORD pid, int signum) {
  if (signum == 0) {
    ...
    HANDLE h = nullptr;
    NTSTATUS st =
        ::NtOpenProcessById(&h, PROCESS_QUERY_LIMITED_INFORMATION, pid);
    if (NT_SUCCESS(st)) {
      ::NtClose(h);
      return 0;
    }
  ...
  case SIGQUIT:
    return send_console_ctrl(console::CTRL_BREAK_EVENT, pid);
...
// ---------------------------------------------------------------------------
intptr_t sigaltstack(const void *ss_arg, void *oss_arg) {
  auto *ss = static_cast<const stack_t *>(ss_arg);
  auto *oss = static_cast<stack_t *>(oss_arg);
...
intptr_t sigsuspend(const sigset_t *mask) {
  ...
  for (;;) {
    ...
    if (state->stop_state.load(cpp::MemoryOrder::ACQUIRE) !=
        signal_state::process_control::STOP_RUNNING) {
      state->waiting_signals.store(0);
...
static int try_consume_signal(signal_state::ThreadSignalState *state,
  ...
  for (int sig = 1; sig < NSIG && ready; ++sig) {
    uint64_t bit = 1ULL << (sig - 1);
    if (!(ready & bit))
      continue;

    // Claim the bit atomically.
    uint64_t old = state->pending.standard.fetch_and(~bit,
  ...
  uint64_t proc_pending =
      g_pcb.signal_dispatch.process_pending.standard.load(
          cpp::MemoryOrder::ACQUIRE);
  uint64_t proc_ready = proc_pending & wait_mask;

  for (int sig = 1; sig < NSIG && proc_ready; ++sig) {
    uint64_t bit = 1ULL << (sig - 1);
    if (!(proc_ready & bit))
      continue;

    uint64_t old = g_pcb.signal_dispatch.process_pending.standard.fetch_and(
        ~bit, cpp::MemoryOrder::ACQ_REL);
    if (!(old & bit))
      continue;
