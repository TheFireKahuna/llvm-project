...
public:
  ...
  [[nodiscard]] LIBC_INLINE SigqueueEntry *try_pop() {
    ...
    if (tail_.load(cpp::MemoryOrder::ACQUIRE) != h) {
      // Incompleteness window: a producer completed tail_.exchange(node)
      // but has not yet executed prev->next.store(node, RELEASE). The
      // next write to this process will land in h->next. Monitor that
      // cache line via UMWAIT C0.2 / MWAITX so the producer's RELEASE
      // store wakes us the moment it retires. UMWAIT C0.2 is near-zero
      // power and doesn't contend SMT resources with the producer core.
      // Low multiplier (256) caps the wait at ~1μs so a preempted
      // producer doesn't stall the consumer — we fall through to the
      // "retry on next drain pass" behavior via the notify::SIGNAL / APC
      // chain once the producer resumes.
      spin_wait::spin_on_raw<SigqueueEntry *>(
          &h->next.val, nullptr, /*tsc_multiplier=*/256);
      next = h->next.load(cpp::MemoryOrder::ACQUIRE);
      if (!next)
        return nullptr; // Producer preempted — next dispatch pass picks it up.
      head_ = next;
      return h;
    }
    ...
    // Stub push is in the incompleteness window — same treatment as above.
    // Our own push(&stub_) just executed tail_.exchange; the next write
    // is prev->next.store for the stub. Since stub_ follows h in the
    // list, the link we need is h->next (which stub's predecessor set).
    spin_wait::spin_on_raw<SigqueueEntry *>(
        &h->next.val, nullptr, /*tsc_multiplier=*/256);
    next = h->next.load(cpp::MemoryOrder::ACQUIRE);
    if (next) {
  ...
  // Conservative emptiness check. Single-consumer only — must be called
  // from the same thread that calls try_pop(), since head_ is consumer-private.
  //
  ...
  //
  // Non-const because cpp::Atomic::load() is non-const (it accesses the
  // internal value_type member). This is safe — load is a read-only
  // operation, and this function is single-consumer by contract.
  LIBC_INLINE bool empty() {
    SigqueueEntry *h = head_;
    if (h == &stub_)
      return h->next.load(cpp::MemoryOrder::ACQUIRE) == nullptr;
    return false;
  }
