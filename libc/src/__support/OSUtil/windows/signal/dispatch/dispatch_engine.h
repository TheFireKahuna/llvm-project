...
//
// Key components:
//   - notify::SIGNAL bit + dispatching bool (replaces old 3-state enum)
...
// ---------------------------------------------------------------------------
//
// Cross-thread notification uses the notify::SIGNAL bit in the owning
// ThreadLifecycle's ThreadLocalWord (notify_word). Transports call
// trigger() which sets the bit via signal_or(). The owning thread checks
// the bit at dispatch boundaries.
//
// Reentrancy prevention uses the bool ThreadSignalState::dispatching
// (owner-only, no atomics). The old three-state atomic enum
// (IDLE/TRIGGERED/DRAINING) is replaced by this two-field design:
//   - notify::SIGNAL bit — cross-thread "you have signals" (in notify_word)
//   - dispatching bool   — owner reentrancy guard (in ThreadSignalState)
...
[[nodiscard]] bool should_restart_syscall(ThreadSignalState *state);
