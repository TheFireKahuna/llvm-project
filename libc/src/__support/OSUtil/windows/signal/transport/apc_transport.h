...
//
// PRECONDITION: The caller must hold an EpochGuard (or otherwise be
// epoch-pinned) to keep the lifecycle and its thread handle alive for the
// duration of the call. Currently only called from send_to_thread() which
// holds the guard.
//
// Returns:
//   0       on success
