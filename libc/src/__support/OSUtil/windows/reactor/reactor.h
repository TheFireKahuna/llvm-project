...
//
// Slots are allocated from an IndexedPool and identified by index rather than
// by pointer. The pool reserves VA for the slot's lifetime and only ever
// discards physical backing via MEM_RESET — pages remain readable (zero-fill
// on reclaim), so a stale completion key whose target slot was freed never
// faults on dereference. The generation counter, incremented on every
// unwatch/detach, is compared against the slot's live generation to reject
// stale completions from previous incarnations of the same index.
//
// idx == 0 is reserved as the INVALID_TOKEN sentinel.
struct ReactorToken {
  uint32_t idx;
  uint32_t generation;

  LIBC_INLINE bool valid() const { return idx != 0; }
};

inline constexpr ReactorToken INVALID_TOKEN = {0, 0};
...
// Registered into `.libcfin$P7` so it walks after the signal/alpc_bus
// finis whose ALPC handler dispatch is reactor-driven.
void fini();
