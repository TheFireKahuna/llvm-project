...
bool nls_init() {
  callonce(&g_pcb.nls.init_flag, nls_init_impl);
  return blob() != nullptr;
}
...
//===----------------------------------------------------------------------===//
// UTF-16 → UTF-8 conversion (uses ntdll RtlUnicodeToUTF8N)
//===----------------------------------------------------------------------===//
...
static int wide_to_utf8(const WCHAR *src, int src_chars, char *dst,
  ...
  NTSTATUS status = ::RtlUnicodeToUTF8N(
      dst, static_cast<ULONG>(dst_len > 0 ? dst_len - 1 : 0), &utf8_bytes,
      src, static_cast<ULONG>(src_chars) * sizeof(WCHAR));
  if (!NT_SUCCESS(status))
    return 0;

  if (dst && dst_len > 0) {
    if (utf8_bytes >= dst_len)
      utf8_bytes = static_cast<ULONG>(dst_len - 1);
    dst[utf8_bytes] = '\0';
  }
  return static_cast<int>(utf8_bytes + 1);
}
...
static bool append_bytes(char *dst, size_t dst_len, size_t *pos,
                         const char *src, size_t src_len) {
  if (!dst || *pos + src_len + 1 > dst_len)
    return false;

  __builtin_memcpy(dst + *pos, src, src_len);
  *pos += src_len;
  return true;
}

static bool append_cstr(char *dst, size_t dst_len, size_t *pos,
                        const char *src) {
  return append_bytes(dst, dst_len, pos, src, __builtin_strlen(src));
}

static bool append_utf8_wchar(char *dst, size_t dst_len, size_t *pos, WCHAR ch) {
  char utf8[4];
  ULONG utf8_bytes = 0;
  NTSTATUS status = ::RtlUnicodeToUTF8N(utf8, sizeof(utf8), &utf8_bytes, &ch,
                                        sizeof(ch));
  if (!NT_SUCCESS(status))
    return false;

  return append_bytes(dst, dst_len, pos, utf8, utf8_bytes);
}

static bool append_literal_wchar(char *dst, size_t dst_len, size_t *pos,
                                 WCHAR ch) {
  if (ch == L'%')
    return append_cstr(dst, dst_len, pos, "%%");
  return append_utf8_wchar(dst, dst_len, pos, ch);
}
...
static int translate_windows_pattern_utf8(const WCHAR *pattern, int len,
  ...
  const bool has_explicit_era = pattern_has_token(pattern, len, L'g');
  size_t pos = 0;
  ...
  for (int i = 0; i < len;) {
    ...
    if (ch == L'\'') {
      if (i + 1 < len && pattern[i + 1] == L'\'') {
        if (!append_literal_wchar(dst, dst_len, &pos, L'\''))
          return 0;
        i += 2;
        continue;
      }
      ...
      while (i < len) {
        ...
        if (!append_literal_wchar(dst, dst_len, &pos, pattern[i]))
          return 0;
        ++i;
      }
    ...
    if (is_pattern_letter(ch)) {
      ...
      if (!token)
        return 0;
      if (!append_cstr(dst, dst_len, &pos, token))
        return 0;
      i += count;
      continue;
    }

    if (!append_literal_wchar(dst, dst_len, &pos, ch))
      return 0;
    ++i;
  }
...
int nls_alt_digits_utf8(const uint8_t *record, char *dst, size_t dst_len) {
  ...
  for (size_t i = 0; i < 10; ++i) {
    if (i != 0 && !append_literal_wchar(dst, dst_len, &pos, L';'))
      return 0;
    if (!append_utf8_wchar(dst, dst_len, &pos, digits[i]))
      return 0;
  }
...
// Strip POSIX codeset suffix: "en_US.UTF-8" → "en_US", "de_DE" → "de_DE".
// Writes the stripped name into dst (must be at least 64 bytes).
// Returns the length of the stripped name (excluding NUL).
static int strip_codeset(const char *name, char *dst, size_t dst_len) {
  int i = 0;
  for (; name[i] && name[i] != '.' && name[i] != '@' &&
         i < static_cast<int>(dst_len) - 1;
       i++)
    dst[i] = name[i];
  dst[i] = '\0';
  return i;
...
const uint8_t *nls_find_locale(const char *name) {
  ...
  if ((name[0] == 'C' && name[1] == '\0') ||
      (name[0] == 'P' && name[1] == 'O' && name[2] == 'S' &&
       name[3] == 'I' && name[4] == 'X' && name[5] == '\0'))
    return nullptr;
  ...
  // Strip codeset suffix for POSIX names.
  char stripped[64];
  strip_codeset(name, stripped, sizeof(stripped));
