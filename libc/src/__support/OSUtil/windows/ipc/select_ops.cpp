...
intptr_t pselect(int nfds, fd_set *__restrict read_set,
  ...
  int alloc_count = nfds > 0 ? nfds : 1;
  internal::ScratchAlloc<struct pollfd> pfds_s(alloc_count);
  internal::ScratchAlloc<int> poll_to_fd_s(alloc_count);
  if (!pfds_s || !poll_to_fd_s)
    return -ENOMEM;
  struct pollfd *pfds = pfds_s.data();
  int *poll_to_fd = poll_to_fd_s.data();
  nfds_t poll_count = 0;
