...
NTSTATUS read_console_input_ex(HANDLE object_handle,
  ...
  // Routing: prefer an explicit io_target (PTY callers pass their
  // session Connection handle since PEB ConsoleHandle is unrelated).
  // Classic CONIN$ callers leave io_target null and pick up PEB
  // ConsoleHandle, which is their process-wide Connection.
  HANDLE console = io_target ? io_target : get_console_handle();
  if (!console)
    return STATUS_INVALID_HANDLE;
...
// success. When `reference` is non-null, \Input is opened relative to it and
// \Output relative to the resulting \Input handle — matching the decoded
// kernelbase helper Function_1800F90C4. Otherwise both open relative to
// connection_root.
NTSTATUS create_condrv_client_handles(CONDRV_CLIENT_HANDLES *handles,
  ...
  HANDLE input_root = reference ? reference : connection_root;
  HANDLE input = nullptr;
  NTSTATUS status = create_condrv_input(&input, input_root, inherit);
  if (!NT_SUCCESS(status))
    return status;

  HANDLE output_root = reference ? input : connection_root;
  HANDLE output = nullptr;
  status = create_condrv_output(&output, output_root, inherit);
  if (!NT_SUCCESS(status)) {
    ::NtClose(input);
    return status;
  }
...
NTSTATUS open_condrv_client_handles(CONDRV_CLIENT_HANDLES *handles,
  ...
  HANDLE input_root = reference ? reference : connection_root;
  HANDLE input = nullptr;
  NTSTATUS status = open_condrv_input(&input, input_root, inherit);
  if (!NT_SUCCESS(status))
    return status;

  HANDLE output_root = reference ? input : connection_root;
  HANDLE output = nullptr;
  status = open_condrv_output(&output, output_root, inherit);
  if (!NT_SUCCESS(status)) {
    ::NtClose(input);
    return status;
  }
