...
static SemRegistry *get_registry() {
  ...
  constexpr WCHAR NAME[] = u"\\BaseNamedObjects\\llvm_libc_sysv_sem_reg";
  UNICODE_STRING us;
  ...
  g_registry_region = windows::SectionRegion::create_named(
      &us, sizeof(SemRegistry), PAGE_READWRITE, nullptr, &st);
  if (!g_registry_region)
    return nullptr;
...
static size_t build_numeric_name(const WCHAR *prefix, size_t prefix_len,
                                 int value, WCHAR *buf,
                                 size_t max_wchars) {
  if (max_wchars < prefix_len + 16)
    return 0;

  for (size_t i = 0; i < prefix_len; ++i)
    buf[i] = prefix[i];

  unsigned int id = static_cast<unsigned int>(value);
  WCHAR digits[11];
  int ndigits = 0;
  if (id == 0) {
    digits[ndigits++] = u'0';
    ...
    while (id > 0) {
      digits[ndigits++] = static_cast<WCHAR>(u'0' + (id % 10));
      id /= 10;
    }
  }
  ...
  size_t total = prefix_len + static_cast<size_t>(ndigits);
  buf[total] = u'\0';
  return total;
}

static size_t build_set_name(int semid, WCHAR *buf, size_t max_wchars) {
  constexpr WCHAR PREFIX[] = u"\\BaseNamedObjects\\llvm_libc_sysv_sem_";
  constexpr size_t PREFIX_LEN = sizeof(PREFIX) / sizeof(WCHAR) - 1;
  return build_numeric_name(PREFIX, PREFIX_LEN, semid, buf, max_wchars);
}

static size_t build_lock_name(int semid, WCHAR *buf, size_t max_wchars) {
  constexpr WCHAR PREFIX[] = u"\\BaseNamedObjects\\llvm_libc_sysv_sem_lock_";
  constexpr size_t PREFIX_LEN = sizeof(PREFIX) / sizeof(WCHAR) - 1;
  return build_numeric_name(PREFIX, PREFIX_LEN, semid, buf, max_wchars);
}
...
static HANDLE create_or_open_named_mutant(int semid) {
  WCHAR name_buf[96];
  size_t name_len = build_lock_name(semid, name_buf, 96);
  if (name_len == 0)
    return nullptr;
...
static bool open_existing_set(int semid, int nsems, HANDLE *section_out,
  ...
  WCHAR name_buf[80];
  size_t name_len = build_set_name(semid, name_buf, 80);
  if (name_len == 0)
    return false;
  ...
  windows::SectionHandle section = windows::SectionHandle::open_named(
      &us, SECTION_MAP_READ | SECTION_MAP_WRITE | SECTION_QUERY, &st);
  if (!section)
    return false;
...
static CachedSemSet *get_cached_set(int semid, int nsems) {
  ...
  lock = create_or_open_named_mutant(semid);
  if (!lock) {
...
static bool process_is_alive(pid_t pid) {
  ...
  NTSTATUS st = ::NtOpenProcessById(
      &process, PROCESS_QUERY_LIMITED_INFORMATION | SYNCHRONIZE,
  ...
  NTSTATUS wait_st = ::NtWaitForSingleObject(process, FALSE, &zero_timeout);
  ::NtClose(process);
...
static int try_semop_locked(SemSlot *slot, SemSetHeader *set,
  ...
  SemValue *values = sem_values(set);
  int snapshots[MAX_SEMS_PER_SET] = {};
  bool touched[MAX_SEMS_PER_SET] = {};
  int undo_delta[MAX_SEMS_PER_SET] = {};
...
intptr_t semget(key_t key, int nsems, int semflg) {
  ...
  HANDLE lock = create_or_open_named_mutant(semid);
  if (!lock) {
    ::NtUnmapViewOfSectionEx(NtCurrentProcess(), shared, 0);
    ::NtClose(section);
    slot->state.store(SEM_SLOT_EMPTY, cpp::MemoryOrder::RELEASE);
  ...
  if (free_cache < MAX_SEM_CACHE) {
    ...
    g_cached_sets[free_cache].section = section;
    g_cached_sets[free_cache].lock = lock;
