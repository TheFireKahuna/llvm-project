...
intptr_t socket(int domain, int type, int protocol) {
  ...
  // Open AFD endpoint (also binds the handle to the reactor IOCP for
  // socket-io completion dispatch — see afd_open_endpoint).
  windows::ScopedNtHandle h;
  NTSTATUS s = afd_open_endpoint(h.put(), domain, base_type, protocol);
  if (!NT_SUCCESS(s))
  ...
  // Switch AFD to non-blocking mode at the kernel level. All blocking
  // decisions are made in userland via afd_ioctl_nonblock / afd_ioctl.
  ...
  afd_ioctl(h.get(), IOCTL_AFD_SET_INFORMATION, &w->info, sizeof(w->info),
            nullptr, 0, &w->iosb);
  ...
  auto result = fd_table.alloc(h.get(), open_flags, 0, FileKind::AfdSocket);
  if (!result)
    return -EMFILE;
...
intptr_t listen(int sockfd, int backlog) {
  ...
  NTSTATUS s = afd_ioctl(h, IOCTL_AFD_START_LISTEN, &w->listen_info,
                         sizeof(w->listen_info), nullptr, 0, &w->iosb);
  if (!NT_SUCCESS(s))
    return -ntstatus_to_errno_socket(s);
...
intptr_t accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
  ...
  // WAIT_FOR_LISTEN — generic, uses listener's scratch. Routed through
  // the reactor IOCP: the listener thread parks on its own notify_word
  // until an inbound connection arrives.
  auto *w = reinterpret_cast<GenericAcceptWflWork *>(socket_scratch(state));
  __builtin_memset(w, 0, sizeof(GenericAcceptWflWork));

  NTSTATUS s;
  if (nonblock) {
    s = afd_ioctl_nonblock(listener, IOCTL_AFD_WAIT_FOR_LISTEN, nullptr, 0,
                           w->wfl_buf, sizeof(w->wfl_buf), &w->iosb);
    if (s == STATUS_DEVICE_NOT_READY)
      return -EAGAIN;
  } else {
    s = afd_ioctl(listener, IOCTL_AFD_WAIT_FOR_LISTEN, nullptr, 0, w->wfl_buf,
                  sizeof(w->wfl_buf), &w->iosb);
  }
  ...
  windows::ScopedNtHandle acc_handle;
  intptr_t result = state->ops->setup_accepted(
      listener, state, sequence, wfl_remote, wfl_remote_len, acc_handle.put(),
  ...
  auto fd_result =
      fd_table.alloc(acc_handle.get(), O_RDWR, 0, FileKind::AfdSocket);
  if (!fd_result)
    return -EMFILE;

  // All committed.
  free_acc_state.dismiss();
  (void)acc_handle.release();
...
intptr_t shutdown(int sockfd, int how) {
  ...
  NTSTATUS s = afd_ioctl(h, IOCTL_AFD_PARTIAL_DISCONNECT, &w->disc,
                         sizeof(w->disc), nullptr, 0, &w->iosb);
  if (!NT_SUCCESS(s))
    return -ntstatus_to_errno_socket(s);
