...
///
/// Serialization: the phase state machine + atomic phase CAS keep at most
/// one lifecycle operation per socket at a time. Ops functions may freely
/// overlay the entire scratch region with their own Work structs. I/O
/// completion is routed through the reactor IOCP via per-IRP IoWaiter
/// instances — see afd_io_completion.h.
struct AddressFamilyOps {
  int domain;
  socklen_t max_addr_len;

  // ─── Capability bits ──────────────────────────────────────────────
  /// True iff this family's AFD transport honours IOCTL_AFD_SEND_MESSAGE /
  /// IOCTL_AFD_RECEIVE_MESSAGE (the datagram/ancillary-data I/O path).
  /// AF_UNIX is routed through afunix.sys, whose 0x1AFD endpoint gate
  /// rejects those IOCTLs — sends and receives go through IOCTL_AFD_SEND /
  /// IOCTL_AFD_RECEIVE instead. AF_INET/AF_INET6 set this true so
  /// sendmsg/recvmsg with msg_name or msg_control routes correctly
  /// without reintroducing a per-family switch in the data plane.
  bool supports_send_message;
