...
static HANDLE get_afd_mio() {
  ...
  name.Buffer = const_cast<WCHAR *>(path);

  auto oa = windows::named_internal_oa(&name);

  HANDLE mio = nullptr;
  IO_STATUS_BLOCK iosb = {};
  NTSTATUS s = NtCreateFile(&mio, SYNCHRONIZE, &oa, &iosb, nullptr, 0,
  ...
  if (!g_afd_mio.compare_exchange_strong(expected, mio,
...
struct SigmaskGuard {
  ...
  ~SigmaskGuard() {
    if (active)
      signal_state::rt_sigprocmask(SIG_SETMASK, &saved_mask, nullptr);
  }
};
} // namespace
...
intptr_t ppoll(struct pollfd *fds, nfds_t nfds, const struct timespec *tmo,
  ...
  if (sigmask) {
    ...
    signal_state::rt_sigprocmask(SIG_SETMASK, sigmask, &guard.saved_mask);
    guard.active = true;
  ...
  if (have_afd) {
    ...
    if (afd_mio) {
      ...
      ::NtCreateEvent(&afd_event, EVENT_MODIFY_STATE | SYNCHRONIZE, &afd_evt_oa,
      ...
      if (afd_event) {
        ...
        NTSTATUS s = ::NtDeviceIoControlFile(
            afd_mio, afd_event, nullptr, nullptr, ws_iosb, IOCTL_AFD_POLL,
            ws_poll_buf, poll_size, ws_poll_buf, poll_size);
        ...
        ws_wait_handles[wait_count] = afd_event;
  ...
  HANDLE poll_iocp = nullptr;
  auto iocp_oa = windows::internal_oa();
  NTSTATUS s = ::NtCreateIoCompletion(&poll_iocp, IO_COMPLETION_ALL_ACCESS,
                                      &iocp_oa, 1);
  if (!NT_SUCCESS(s))
    return -ENOMEM;

  auto cleanup_iocp = cpp::make_scope_guard([&] { ::NtClose(poll_iocp); });
  ...
  for (int i = 0; i < wait_count; ++i) {
    ...
    s = ::NtAssociateWaitCompletionPacket(
        wcp, poll_iocp, ws_wait_handles[i],
  ...
  s = ::NtRemoveIoCompletionEx(poll_iocp, &completion, 1, &removed,
