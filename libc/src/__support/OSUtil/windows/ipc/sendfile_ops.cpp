...
ssize_t sendfile(int out_fd, int in_fd, off_t *offset, size_t count) {
  ...
  if (!out_ofd->is_socket()) {
    ...
    if (seekable_out && !is_append) {
      ...
      NTSTATUS es = ::NtCreateEvent(&evt, EVENT_MODIFY_STATE | SYNCHRONIZE,
    ...
    if (use_kernel_copy) {
      while (total < count) {
        ...
        NTSTATUS s = ::NtCopyFileChunk(in_h, out_h, evt, &iosb, chunk,
                                       &src_off, &dst_off,
                                       nullptr, nullptr, 0);
        if (s == STATUS_PENDING) {
  ...
  if (out_ofd->kind == FileKind::SocketPair) {
    ...
    HANDLE evt = nullptr;
    auto evt_oa = windows::internal_oa();
    NTSTATUS es = NtCreateEvent(&evt, EVENT_MODIFY_STATE | SYNCHRONIZE, &evt_oa,
    ...
    while (total < count) {
      ...
      NTSTATUS s = NtReadFile(in_h, evt, nullptr, nullptr, &iosb, buf,
  ...
  for (;;) {
    iosb = {};
    s = afd_ioctl(out_h, evt, IOCTL_AFD_TRANSMIT_FILE, &tfi,
                  sizeof(tfi), nullptr, 0, &iosb);
