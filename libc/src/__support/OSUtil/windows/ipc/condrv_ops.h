...
LIBC_INLINE NTSTATUS open_condrv_server(
    ...
    bool inherit = true) {
  windows::nt_wstring_view name(CONDRV_SERVER_PATH);
...
// When `reference` is non-null the client input/output handles are opened
// using the kernelbase-decoded hierarchy: \Input rel Reference, \Output rel
// Input. When null, both open relative to connection_root (legacy path).
// kernelbase's Function_1800F90C4 uses the reference-rooted variant.
...
LIBC_INLINE NTSTATUS open_condrv_input(HANDLE *out, HANDLE connection_root,
                                       bool inherit = true) {
  windows::nt_wstring_view name(CONDRV_INPUT_NAME);
...
LIBC_INLINE NTSTATUS open_condrv_output(HANDLE *out, HANDLE connection_root,
                                        bool inherit = true) {
  windows::nt_wstring_view name(CONDRV_OUTPUT_NAME);
