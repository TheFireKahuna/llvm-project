...
LIBC_INLINE size_t derive_pipe_name(const char *path, WCHAR *out,
  ...
  auto nt = to_nt_path(path_sv, canon, canon_s.size());
  if (!nt.has_value())
    return 0;
  size_t canon_len = nt.value();

  // Case-fold for consistency (NT paths are case-insensitive).
  for (size_t i = 0; i < canon_len; ++i) {
    if (canon[i] >= u'A' && canon[i] <= u'Z')
      canon[i] += 32;
  ...
  ss << windows::nt_wstring_view(FIFO_PIPE_PREFIX, FIFO_PIPE_PREFIX_LEN);
...
LIBC_INLINE size_t derive_fifo_hash(const char *path, WCHAR *out,
  ...
  if (hash_len >= max_wchars)
    return 0;
  __builtin_memcpy(out, full + FIFO_PIPE_PREFIX_LEN, hash_len * sizeof(WCHAR));
  out[hash_len] = u'\0';
  return hash_len;
}
