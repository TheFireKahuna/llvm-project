...
struct InotifyWatch {
  ...
  bool dont_follow;     // IN_DONT_FOLLOW was set — preserve across fork.
  ...
  // NT path saved at add_watch time, used by fork_reinit_instance to
  // reopen the directory in the child (the parent's dir_handle was
  // created non-inheritable and does not survive fork). Owned by the
  // watch — page_alloc'd, page_free'd in inotify_watch_free. UTF-16,
  // NOT null-terminated. saved_path_len is a WCHAR count.
  //
  // Typed as uint16_t rather than wchar_t because NTPOSIX compiles with
  // -fwchar-type=int -fsigned-wchar, so wchar_t is 32-bit there. Windows
  // paths are always UTF-16 (16-bit code units).
  uint16_t *saved_path;
  uint32_t saved_path_len;
...
LIBC_INLINE void inotify_watch_free(InotifyWatch *w) {
  ...
  // 4. Release the saved fork-reopen path.
  if (w->saved_path)
    page_free(w->saved_path);
  page_free(w);
}
