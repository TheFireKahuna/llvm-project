...
//
//   Section  (\Sessions\<N>\BaseNamedObjects\lf-<hash>)      — ring buffer + header
//   Event    (\Sessions\<N>\BaseNamedObjects\lf-<hash>-r)    — "readable" (manual-reset)
//   Event    (\Sessions\<N>\BaseNamedObjects\lf-<hash>-w)    — "writable" (manual-reset)
//   Mutant   (\Sessions\<N>\BaseNamedObjects\lf-<hash>-wm)   — write lock (PIPE_BUF atomicity)
//   Mutant   (\Sessions\<N>\BaseNamedObjects\lf-<hash>-rm)   — read lock (multi-reader safety)
//
...
LIBC_INLINE bool init_named_oa(OBJECT_ATTRIBUTES *oa, windows::nt_wstring_view *wsv,
  ...
  windows::WStringStream ss(cpp::span<WCHAR>(buf, buf_wchars - 1));
  windows::write_session_bno_prefix(ss);
  ss << u"lf-"
     << windows::nt_wstring_view(base_name, base_len)
     << windows::nt_wstring_view(suffix, suffix_len);
  if (ss.overflow())
    return false;
  ss.null_terminate();
  size_t total = ss.str().size();

  *wsv = windows::nt_wstring_view(buf, total);
  *oa = windows::named_internal_oa(wsv, nullptr, sd);
  return true;
}
...
LIBC_INLINE FifoChannel *fifo_open_channel(const WCHAR *hash_name,
  ...
  if (!name_s)
    return nullptr;
  WCHAR *name_buf = name_s.data();
  size_t name_buf_wchars = name_s.size();
  windows::nt_wstring_view wsv;
  ...
  // --- Section ---
  if (!init_named_oa(&oa, &wsv, name_buf, name_buf_wchars, hash_name, hash_len, u"", 0, sd))
    return nullptr;

  h.section = windows::SectionHandle::create_named(
      &wsv, FIFO_SECTION_SIZE,
  ...
  NTSTATUS status;
  if (!init_named_oa(&oa, &wsv, name_buf, name_buf_wchars, hash_name, hash_len,
                      u"-r", 2, sd)) {
    h.close_all();
    return nullptr;
  }
...
LIBC_INLINE ssize_t fifo_read(FifoChannel *ch, void *buf, size_t count,
  ...
  for (;;) {
    ...
    if (rp < wp) {
      ...
      NtReleaseMutant(ch->read_mutant, nullptr);
      // TODO(futex_addr-wake-race): wake-all until the DLL+tail single-pass
      // wake redesign lands (see FUTEX_ADDR_WAKE_DESIGN.md). wake(addr, 1)
      // with M>1 parked waiters can silently drop the wake when a waiter
      // races IN_KERNEL->TIMED_OUT between the two passes, leaving a writer
      // parked forever on shared-pipe fan-in. wake-all re-contends on the
      // write mutant; losers re-park. Extra wake cost only.
      futex_addr::wake(&hdr->read_pos, UINT32_MAX);
...
LIBC_INLINE ssize_t fifo_write(FifoChannel *ch, const void *buf, size_t count,
  ...
  while (written < count) {
    ...
    // Write path: take whatever space is available (or the full chunk, if
    // atomic and it fits). Atomic writes (count <= PIPE_BUF) are all-or-
    // nothing — if the remaining payload doesn't fit, fall through to wait.
    const size_t remaining = count - written;
    const bool atomic_blocks = atomic && remaining > space;
    if (space > 0 && !atomic_blocks) {
      ...
      // TODO(futex_addr-wake-race): wake-all — see matching comment in
      // fifo_read above. Remove once futex_addr::wake single-pass tail-first
      // redesign is in.
      futex_addr::wake(&hdr->write_pos, UINT32_MAX);
      // Readiness signal for epoll/poll WCP.
      NtSetEvent(ch->readable_event, nullptr);
      continue;
    }
