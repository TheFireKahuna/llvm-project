...
enum FILE_INFORMATION_CLASS {
  ...
  // Returns the PIDs of every process that currently has a handle open to
  // the file. Used as the kernel-sourced directory for peer discovery in
  // cross-process fcntl lock ownership attestation. Requires
  // FILE_READ_ATTRIBUTES on the handle.
...
// FileProcessIdsUsingFileInformation — list of PIDs that currently have
// a handle open to the file. Variable-length: header + flexible array.
// Each `ProcessIdList[i]` is a ULONG_PTR carrying a DWORD process id.
struct FILE_PROCESS_IDS_USING_FILE_INFORMATION {
  ULONG     NumberOfProcessIdsInList;
  ULONG_PTR ProcessIdList[1]; // actually NumberOfProcessIdsInList entries
};
...
struct FILE_FS_ATTRIBUTE_INFORMATION {
  ...
  WCHAR FileSystemName[1];          // u"NTFS", u"ReFS", u"FAT32", etc.
};
