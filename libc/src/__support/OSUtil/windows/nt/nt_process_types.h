...
//===----------------------------------------------------------------------===//
// Process Cloning — RtlCloneUserProcess
//===----------------------------------------------------------------------===//

// RTL_USER_PROCESS_INFORMATION — output from RtlCloneUserProcess and
// RtlCreateUserProcess.  Contains the process/thread handles, client IDs,
// and image metadata for the newly created process.
//
// We define ImageInformation as an opaque 64-byte array rather than
// pulling in the full SECTION_IMAGE_INFORMATION definition from
// nt_memory_types.h — fork only needs the handles and ClientId.
struct RTL_USER_PROCESS_INFORMATION {
  ULONG Length;           // +0x00  sizeof(this struct), set by caller
  HANDLE ProcessHandle;   // +0x08  handle to the new process
  HANDLE ThreadHandle;    // +0x10  handle to the initial thread
  CLIENT_ID ClientId;     // +0x18  UniqueProcess + UniqueThread
  UCHAR ImageInformation[64]; // +0x28  opaque SECTION_IMAGE_INFORMATION
};
static_assert(sizeof(RTL_USER_PROCESS_INFORMATION) == 104,
              "RTL_USER_PROCESS_INFORMATION must be 104 bytes");
static_assert(FIELD_OFFSET(RTL_USER_PROCESS_INFORMATION, ProcessHandle) == 0x08);
static_assert(FIELD_OFFSET(RTL_USER_PROCESS_INFORMATION, ThreadHandle) == 0x10);
static_assert(FIELD_OFFSET(RTL_USER_PROCESS_INFORMATION, ClientId) == 0x18);
static_assert(FIELD_OFFSET(RTL_USER_PROCESS_INFORMATION, ImageInformation) ==
              0x28);

// RtlCloneUserProcess flags — control the clone behavior.
inline constexpr ULONG RTL_CLONE_PROCESS_FLAGS_CREATE_SUSPENDED = 0x00000001;
inline constexpr ULONG RTL_CLONE_PROCESS_FLAGS_INHERIT_HANDLES = 0x00000002;
inline constexpr ULONG RTL_CLONE_PROCESS_FLAGS_NO_SYNCHRONIZE = 0x00000008;
...
// Win11+: blocks the loader from honouring filesystem junctions /
// symlinks that cross trust boundaries during DLL resolution. Closes
// the planting class where a low-integrity writer plants a junction
// in a location consulted by a higher-integrity loader.
...
// Slot limits.
inline constexpr unsigned FLS_BUCKET_COUNT = 8;

// Per-thread FLS data block (TEB->FlsData).
//
// Decoded from RtlFlsSetValue / RtlFlsGetValue disassembly (Win11 22631+).
// NOT a flat array — bucket-based radix structure.  User FLS index N maps
// to internal index (N + 16); bucket = BSR(internal) - 4, slot within
// bucket = internal XOR (1 << BSR(internal)).
//
// Each Bucket is a separate heap allocation: [8-byte header][PVOID slots...].
// Slot values start at Bucket[1], not Bucket[0] — the first PVOID is a
// header (confirmed by Wine's fls_data_chunks[i][idx + 1] access pattern
// and ntdll's 0x8(%rbp,%rdi,8) addressing in RtlFlsSetValue).
// Bucket sizes are powers of two: bucket k has (1 << (k+4)) slots.
//
// The Link field chains all threads' RTL_FLS_DATA blocks into a process-wide
// circular doubly-linked list headed by FLS_MANAGER::ListHead.
struct RTL_FLS_DATA {
  LIST_ENTRY Link;                 // +0x00  in FLS_MANAGER.ListHead
  PVOID Buckets[FLS_BUCKET_COUNT]; // +0x10  lazily allocated slot arrays
  ULONGLONG Flags;                  // +0x50  bit 0 = dirty
};
static_assert(sizeof(RTL_FLS_DATA) == 0x58,
              "RTL_FLS_DATA must be 88 bytes");
static_assert(FIELD_OFFSET(RTL_FLS_DATA, Link) == 0x00);
static_assert(FIELD_OFFSET(RTL_FLS_DATA, Buckets) == 0x10);
static_assert(FIELD_OFFSET(RTL_FLS_DATA, Flags) == 0x50);

// Process-wide FLS manager (ntdll-private global, since 19H1).
//
// Decoded from RtlFlsAlloc / RtlFlsSetValue / RtlProcessFlsData
// disassembly (Win11 22631+).  All FLS mutations acquire Lock exclusive.
// RtlFlsGetValue is lockless (reads Buckets from per-thread RTL_FLS_DATA).
//
// Prior to 19H1 the list head and bitmap lived in the PEB; since 19H1
// the entire manager is in ntdll's .data section (PEB fields became
// SparePointers[2]).
//
// Layout was determined by cross-referencing RIP-relative accesses in
// RtlFlsAlloc (which loads &Lock into RCX at entry), RtlFlsSetValue
// (which links RTL_FLS_DATA at ListHead+0x48 from Lock), and
// RtlProcessFlsData (which unlinks under Lock).
struct FLS_MANAGER {
  SRWLOCK Lock;                         // +0x00
  PVOID AllocBuckets[FLS_BUCKET_COUNT]; // +0x08  per-bucket callback+alloc tables
  LIST_ENTRY ListHead;                  // +0x48  head of RTL_FLS_DATA list
  ULONG HighIndex;                      // +0x58  highest allocated FLS index
};
static_assert(FIELD_OFFSET(FLS_MANAGER, Lock) == 0x00);
static_assert(FIELD_OFFSET(FLS_MANAGER, AllocBuckets) == 0x08);
static_assert(FIELD_OFFSET(FLS_MANAGER, ListHead) == 0x48);
static_assert(FIELD_OFFSET(FLS_MANAGER, HighIndex) == 0x58);
...
LIBC_MSABI void LDR_DLL_NOTIFICATION_FUNCTION_PROTO(
    ULONG Reason, const LDR_DLL_NOTIFICATION_DATA *Data, PVOID Context);
using LDR_DLL_NOTIFICATION_FUNCTION_FN =
    decltype(LDR_DLL_NOTIFICATION_FUNCTION_PROTO);
using LDR_DLL_NOTIFICATION_FUNCTION = LDR_DLL_NOTIFICATION_FUNCTION_FN *;
