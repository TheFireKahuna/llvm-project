...
__declspec(dllimport) NTSTATUS NTAPI NtCreateProcessStateChange(
...
__declspec(dllimport) NTSTATUS NTAPI NtAlertThreadByThreadId(HANDLE ThreadId);
...
__declspec(dllimport) NTSTATUS NTAPI NtAlertThread(HANDLE ThreadHandle);
...
__declspec(dllimport) void NTAPI RtlWakeAddressSingle(PVOID Address);
// RtlWakeAddressAll — wake all threads waiting on Address.
__declspec(dllimport) void NTAPI RtlWakeAddressAll(PVOID Address);

// Flush CFG/ACG secure memory cache. Called by VirtualProtectEx on
// STATUS_INVALID_PAGE_PROTECTION (0xc0000045) before retrying.
__declspec(dllimport) BOOLEAN NTAPI RtlFlushSecureMemoryCache(PVOID Address,
                                                              SIZE_T Size);
...
__declspec(dllimport) NTSTATUS NTAPI NtCreateThreadStateChange(
...
__declspec(dllimport) NTSTATUS NTAPI NtDuplicateObject(
...
__declspec(dllimport) PVOID NTAPI RtlCreateHeap(ULONG Flags, PVOID HeapBase,
                                                SIZE_T ReserveSize,
...
__declspec(dllimport) PVOID NTAPI RtlAllocateHeap(PVOID HeapHandle, ULONG Flags,
                                                  SIZE_T Size);
...
__declspec(dllimport) PVOID NTAPI RtlReAllocateHeap(PVOID HeapHandle,
                                                    ULONG Flags,
                                                    PVOID BaseAddress,
                                                    SIZE_T Size);
...
__declspec(dllimport) SIZE_T NTAPI RtlSizeHeap(PVOID HeapHandle, ULONG Flags,
                                               PVOID BaseAddress);

// RtlFreeHeap — free an allocation. Returns TRUE on success.
// BaseAddress nullptr is a valid no-op (like free(NULL)).
__declspec(dllimport) BOOL NTAPI RtlFreeHeap(PVOID HeapHandle, ULONG Flags,
                                             PVOID BaseAddress);
...
__declspec(dllimport) NTSTATUS NTAPI NtGetNextProcess(HANDLE ProcessHandle,
...
__declspec(dllimport) NTSTATUS NTAPI NtTerminateProcess(HANDLE ProcessHandle,
                                                        NTSTATUS ExitStatus);
...
__declspec(dllimport) NTSTATUS NTAPI NtTerminateThread(HANDLE ThreadHandle,
                                                       NTSTATUS ExitStatus);
...
__declspec(dllimport) NTSTATUS NTAPI NtResumeThread(HANDLE ThreadHandle,
...
__declspec(dllimport) NTSTATUS NTAPI NtGetContextThread(HANDLE ThreadHandle,
                                                        CONTEXT *ThreadContext);
__declspec(dllimport) NTSTATUS NTAPI NtSetContextThread(HANDLE ThreadHandle,
                                                        CONTEXT *ThreadContext);

// Yield the current thread's remaining quantum. Returns
// STATUS_NO_YIELD_PERFORMED if no other runnable thread was available.
__declspec(dllimport) NTSTATUS NTAPI NtYieldExecution(void);
...
__declspec(dllimport) NTSTATUS NTAPI NtCreateProcessEx(
...
__declspec(dllimport) NTSTATUS NTAPI RtlCreateProcessParametersEx(
...
__declspec(dllimport) NTSTATUS NTAPI NtQueryInformationThread(
...
__declspec(dllimport) NTSTATUS NTAPI NtSetTimerResolution(ULONG DesiredTime,
...
__declspec(dllimport) NTSTATUS NTAPI NtSetEvent(HANDLE EventHandle,
                                                PLONG PreviousState);
...
__declspec(dllimport) NTSTATUS NTAPI NtSetEventEx(HANDLE EventHandle,
                                                  PRTL_SRWLOCK Lock);

// NtResetEvent - unsignal a manual-reset event.
// PreviousState receives the state before the reset (may be nullptr).
__declspec(dllimport) NTSTATUS NTAPI NtResetEvent(HANDLE EventHandle,
                                                  PLONG PreviousState);
...
__declspec(dllimport) NTSTATUS NTAPI NtClearEvent(HANDLE EventHandle);
...
__declspec(dllimport) NTSTATUS NTAPI NtPulseEvent(HANDLE EventHandle,
                                                  PLONG PreviousState);
...
__declspec(dllimport) NTSTATUS NTAPI NtQueryEvent(
...
__declspec(dllimport) NTSTATUS NTAPI NtDelayExecution(BOOLEAN Alertable,
...
__declspec(dllimport) NTSTATUS NTAPI NtCreateTimer2(
...
__declspec(dllimport) NTSTATUS NTAPI NtSetTimer2(HANDLE TimerHandle,
...
__declspec(dllimport) NTSTATUS NTAPI NtCancelTimer2(HANDLE TimerHandle,
                                                    PVOID Parameters);

// NtQueryTimer — query remaining time and state.
__declspec(dllimport) NTSTATUS NTAPI NtQueryTimer(
...
__declspec(dllimport) NTSTATUS NTAPI NtReleaseMutant(HANDLE MutantHandle,
                                                     PLONG PreviousCount);
...
__declspec(dllimport) NTSTATUS NTAPI NtCreateSemaphore(
...
__declspec(dllimport) NTSTATUS NTAPI NtReleaseSemaphore(HANDLE SemaphoreHandle,
                                                        LONG ReleaseCount,
                                                        PLONG PreviousCount);
...
__declspec(dllimport) NTSTATUS NTAPI LdrUnregisterDllNotification(PVOID Cookie);
...
__declspec(dllimport) void NTAPI RtlProcessFlsData(PVOID FlsData, ULONG Flags);
...
__declspec(dllimport) NTSTATUS NTAPI NtSetInformationObject(
...
__declspec(dllimport) NTSTATUS NTAPI NtMakeTemporaryObject(HANDLE Handle);
...
__declspec(dllimport) NTSTATUS NTAPI NtMakePermanentObject(HANDLE Handle);

//===----------------------------------------------------------------------===//
// Private Namespaces — isolated object namespaces for sem_open/shm_open
//===----------------------------------------------------------------------===//

__declspec(dllimport) OBJECT_BOUNDARY_DESCRIPTOR *NTAPI
...
__declspec(dllimport) NTSTATUS NTAPI RtlAddSIDToBoundaryDescriptor(
...
__declspec(dllimport) NTSTATUS NTAPI NtAllocateUuids(ULARGE_INTEGER *Time,
                                                     ULONG *Range,
                                                     ULONG *Sequence,
                                                     CHAR *Seed);
...
__declspec(dllimport) NTSTATUS NTAPI CsrClientConnectToServer(
