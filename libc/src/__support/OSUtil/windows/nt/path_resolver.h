...
LIBC_INLINE void write_nt_prefix(WCHAR *buf) {
  static constexpr WCHAR PREFIX[] = u"\\??\\";
  __builtin_memcpy(buf, PREFIX, NT_PREFIX_LEN * sizeof(WCHAR));
}
...
LIBC_INLINE WideResult utf8_to_wide_nt(const char *src, size_t src_len,
  ...
  int result = LIBC_NAMESPACE::windows::utf8_to_utf16(
      src, src_len, buf + offset, max_wchars - offset);
  if (result < 0)
    return {0, result == -ENAMETOOLONG ? ENAMETOOLONG : EILSEQ};

  size_t chars = static_cast<size_t>(result);
...
// ---------------------------------------------------------------------------
LIBC_INLINE PathKind classify_path(LIBC_NAMESPACE::cpp::string_view sv) {
  if (sv.empty())
...
// ---------------------------------------------------------------------------
LIBC_INLINE int resolve_dev_fd(LIBC_NAMESPACE::cpp::string_view sv) {
  if (!sv.starts_with("/dev/"))
    return -1;
  auto name = sv.substr(5);
...
LIBC_INLINE path_detail::WideResult resolve_dev_device(PathKind kind,
                                                       WCHAR *buf,
                                                       size_t max_wchars) {
  LIBC_NAMESPACE::windows::WStringStream ss(
      LIBC_NAMESPACE::cpp::span<WCHAR>(buf, max_wchars));
...
resolve_tmp_path(LIBC_NAMESPACE::cpp::string_view sv, WCHAR *buf,
                 size_t max_wchars) {
...
// ---------------------------------------------------------------------------
LIBC_INLINE path_detail::WideResult
resolve_posix_absolute(LIBC_NAMESPACE::cpp::string_view path, WCHAR *buf,
                       size_t max_wchars) {
  using namespace path_detail;
  ...
  auto rest = path.substr(1);

  auto wr = utf8_to_wide_nt(rest.data(), rest.size(), buf,
                            NT_PREFIX_LEN + DRIVE_LEN,
                            max_wchars);
  if (wr.error)
    return wr;
...
// ---------------------------------------------------------------------------
LIBC_INLINE path_detail::WideResult
resolve_drive_relative(LIBC_NAMESPACE::cpp::string_view path, WCHAR *buf,
                       size_t max_wchars) {
  using namespace path_detail;
  ...
  // Scratch buffer for the per-drive CWD. PATH_SCRATCH_WCHARS handles
  // any path length including long-path-aware environments.
  auto drive_cwd_s = LIBC_NAMESPACE::windows::path_scratch();
  if (!drive_cwd_s)
    return {0, ENOMEM};
  WCHAR *drive_cwd = drive_cwd_s.data();
  size_t drive_cwd_cap = drive_cwd_s.size();
  ...
  if (cwd_chars >= 2) {
    ...
    if (cwd_drive == drive_upper) {
      ...
      if (drive_cwd_len >= drive_cwd_cap)
        return {0, ENAMETOOLONG};
  ...
  if (drive_cwd_len > 0 && drive_cwd[drive_cwd_len - 1] != u'\\') {
    if (drive_cwd_len + 1 >= drive_cwd_cap)
      return {0, ENAMETOOLONG};
  ...
  auto wr = utf8_to_wide_nt(relative.data(), relative.size(), buf, rel_offset,
                            max_wchars);
  if (wr.error)
    return wr;
...
// ---------------------------------------------------------------------------
LIBC_INLINE path_detail::WideResult
resolve_rooted_path(LIBC_NAMESPACE::cpp::string_view path, WCHAR *buf,
                    size_t max_wchars) {
  using namespace path_detail;
  ...
  auto wr = utf8_to_wide_nt(path.data(), path.size(), buf,
                            NT_PREFIX_LEN + DRIVE_LEN,
                            max_wchars);
  if (wr.error)
    return wr;
...
// ---------------------------------------------------------------------------
LIBC_INLINE path_detail::WideResult
resolve_dos_absolute(LIBC_NAMESPACE::cpp::string_view path, WCHAR *buf,
                     size_t max_wchars) {
  using namespace path_detail;

  auto wr = utf8_to_wide_nt(path.data(), path.size(), buf, NT_PREFIX_LEN,
                            max_wchars);
  if (wr.error)
    return wr;
...
// ---------------------------------------------------------------------------
LIBC_INLINE path_detail::WideResult
resolve_relative(LIBC_NAMESPACE::cpp::string_view path, WCHAR *buf,
                 size_t max_wchars) {
  using namespace path_detail;
...
// ---------------------------------------------------------------------------
LIBC_INLINE path_detail::WideResult
resolve_unc(LIBC_NAMESPACE::cpp::string_view path, WCHAR *buf,
            size_t max_wchars) {
  using namespace path_detail;
...
// ---------------------------------------------------------------------------
LIBC_INLINE path_detail::WideResult
resolve_verbatim_or_device(LIBC_NAMESPACE::cpp::string_view path, WCHAR *buf,
                           size_t max_wchars) {
  using namespace path_detail;
...
// ---------------------------------------------------------------------------
LIBC_INLINE int validate_path_length(LIBC_NAMESPACE::cpp::string_view sv) {
  // Null/empty checks are handled by the caller before constructing the
  // string_view. This function validates length only.
  ...
  if (sv.size() > MAX_NT_PATH_WCHARS)
    return ENAMETOOLONG;
  return 0;
}

// ---------------------------------------------------------------------------
// resolve_path — internal implementation of full path resolution.
//
...
//
// Most callers should use to_nt_path() from nt_path_convert.h instead,
// which wraps this and returns ErrorOr<size_t>. Direct use of resolve_path
// is only needed when the caller must dispatch on PathKind (e.g. openat
// handling /dev/ptmx, /dev/fd/N, etc.).
//
// Null checking is the caller's responsibility.
// ---------------------------------------------------------------------------
namespace path_detail {
LIBC_INLINE ResolvedPath resolve_path(LIBC_NAMESPACE::cpp::string_view sv,
                                      WCHAR *buf, size_t max_wchars) {
  if (sv.empty())
    return {PathKind::Invalid, ENOENT, 0, -1};

  int verr = validate_path_length(sv);
  if (verr)
  ...
  case PathKind::PosixTmp:
    return from_wide(resolve_tmp_path(sv, buf, max_wchars));
  case PathKind::PosixAbsolute:
    return from_wide(resolve_posix_absolute(sv, buf, max_wchars));
  case PathKind::DosDriveRelative:
    return from_wide(resolve_drive_relative(sv, buf, max_wchars));
  case PathKind::DosRooted:
    return from_wide(resolve_rooted_path(sv, buf, max_wchars));
  case PathKind::DosAbsolute:
    return from_wide(resolve_dos_absolute(sv, buf, max_wchars));
  case PathKind::DosRelative:
    return from_wide(resolve_relative(sv, buf, max_wchars));
  case PathKind::Unc:
    return from_wide(resolve_unc(sv, buf, max_wchars));
  case PathKind::VerbatimOrDevice:
    return from_wide(resolve_verbatim_or_device(sv, buf, max_wchars));
  }
