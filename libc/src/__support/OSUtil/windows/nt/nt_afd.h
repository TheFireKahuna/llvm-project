...
#define AFD_ENDPOINT_PATH L"\\Device\\Afd\\Endpoint"
...
#define AFD_MIO_PATH L"\\Device\\Afd\\Mio"
...
// afunix.sys private socket-ioctl opcodes recovered from its transport
// dispatcher. The current Win11 path reaches these through the AF_UNIX
// transport-control wrapper; 0x98000001 is accepted on the traced
// TlGetSockOptIoControlType/TlSocketIoControlType corridor and stored for
// later bind-time use.
inline constexpr ULONG AFD_AFINUX_SET_FILE_PATH = 0x98000000;
// Stores a pending bind-side EA buffer on the endpoint. afunix.sys does not
// parse the inner bytes; bind later forwards them unchanged as the
// EaBuffer/EaLength pair to IoCreateFileEx.
...
// Private AF_UNIX endpoint ioctls issued through TlEndpointIoControlType
// inside afunix.sys. Current Win11 tracing resolves the full inner switch as:
//   0x04 = AfUnixEndpointIoctlCancelIo
//   0x08 = AfUnixEndpointIoctlBind
//   0x0C = AfUnixEndpointIoctlUnbind (stubbed: logs "unbind unimplemented"
//          and returns STATUS_INVALID_PARAMETER on the traced build)
//   0x10 = AfUnixEndpointIoctlQueryLocalAddress
//   0x14 = AfUnixEndpointIoctlQueryRemoteAddress
// The `0x08` bind arm takes a sockaddr_un-like buffer, rejects short/non-AF_UNIX
// input, uses provider-primed pathname state only for pathname binds, and
// publishes the resulting local address into the endpoint's local query slot.
// For pathname binds, the backing file create/open returns a referenced
// FILE_OBJECT. afunix.sys keys duplicate binds and later open-to-endpoint lookup
// on `FILE_OBJECT.SectionObjectPointer ?: FILE_OBJECT.FsContext`.
// A straight user-mode IOCTL_AFD_TRANSPORT_IOCTL probe with
// TlEndpointIoControlType was rejected with STATUS_INVALID_PARAMETER on the
// traced Windows 11 path, so libc should treat these as internal provider
// operations reached via the mswsock/wshunix bind/connect bridge rather than a
// stable direct user ABI.
...
inline constexpr ULONG AFD_UNIX_CONTEXT_MIN_SELECTOR_LENGTH = 0x80;
inline constexpr ULONG AFD_UNIX_CONTEXT_SELECTOR_LENGTH =
    AFD_UNIX_CONTEXT_MIN_SELECTOR_LENGTH;
inline constexpr ULONG AFD_UNIX_SELECTOR_OPAQUE_DATA_LENGTH =
    AFD_UNIX_CONTEXT_SELECTOR_LENGTH - sizeof(LONG);
...
// The AF_UNIX selector slot is not decoded by afd.sys. AfdSetContext copies
// these bytes verbatim into the endpoint's cached query blob and AfdGetContext
// returns them unchanged.
//
// The first 0x80 bytes are the minimum publication header: either a minimal
// pure-NT selector cookie, or the fixed header of the larger Win11 wshunix
// snapshot. Win32/mswsock does publish selector blobs longer than 0x80 by
// setting SelectorBlobLength to the full
// WSHGetSocketInformation(level=0xFFFE, option=1) result size:
// 0x80 + LocalPathByteCount + RemotePathByteCount.
//
// The durable interpretation AFD can be trusted with is therefore only the
// leading AF_UNIX marker. Any deeper semantics belong to the user-mode helper
// snapshot carried in the variable-length bytes.
struct AFD_UNIX_SELECTOR_BLOB {
  LONG AddressFamilyValue; // +0x00 = AF_UNIX
  UCHAR ProviderOpaque[AFD_UNIX_SELECTOR_OPAQUE_DATA_LENGTH];
};
...
using PIO_MINI_COMPLETION_PACKET_CALLBACK = void(NTAPI *)(PVOID PacketArgument,
                                                          PVOID Context);
