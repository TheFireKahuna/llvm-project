...
//===----------------------------------------------------------------------===//
// TEB slot accessors — the TLS index is stored in sealed Zone 0
// (g_pcb.zone0.veh_sealed().fault_guard_tls_index) and passed in from
// the master handler / call-site code.
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// Convenience overloads — read TLS index from sealed Zone 0 automatically.
//===----------------------------------------------------------------------===//

[[gnu::returns_twice]]
LIBC_INLINE bool fault_guard_enter(FaultGuard *g, uint32_t mask) {
  return fault_guard_enter(g, g_pcb.zone0.veh_sealed().fault_guard_tls_index,
                           mask);
}
