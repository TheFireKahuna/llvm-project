...
LIBC_INLINE void *page_reserve(size_t size) {
  void *addr = nullptr;
  SIZE_T sz = size;
...
LIBC_INLINE bool page_commit(void *addr, size_t size) {
  SIZE_T sz = size;
  NTSTATUS st = ::NtAllocateVirtualMemoryEx(
...
LIBC_INLINE bool page_commit_numa(void *addr, size_t size, int numa_node) {
  SIZE_T sz = size;
  if (numa_node < 0)
...
LIBC_INLINE bool page_reset_undo(void *addr, size_t size) {
  SIZE_T sz = size;
...
LIBC_INLINE void *page_alloc(size_t size) {
  void *addr = nullptr;
  SIZE_T sz = size;
  NTSTATUS st = ::NtAllocateVirtualMemoryEx(
...
LIBC_INLINE void *page_alloc_numa(size_t size, int numa_node) {
  if (numa_node < 0)
...
LIBC_INLINE void *page_reserve_at(void *addr, size_t size) {
  PVOID base = addr;
  SIZE_T sz = size;
  NTSTATUS st = ::NtAllocateVirtualMemoryEx(
...
LIBC_INLINE void *placeholder_reserve(size_t size) {
  void *addr = nullptr;
  SIZE_T sz = size;
  NTSTATUS st = ::NtAllocateVirtualMemoryEx(
...
LIBC_INLINE bool placeholder_commit(void *addr, size_t size) {
  SIZE_T sz = size;
  NTSTATUS st = ::NtAllocateVirtualMemoryEx(
      NtCurrentProcess(), &addr, &sz,
