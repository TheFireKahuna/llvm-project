...
public:
  ...
  LIBC_INLINE void unmap_release() {
    if (base_) {
      ...
      // Unmap failure indicates a logic bug (double-unmap, wrong address).
      // Trap unconditionally — LIBC_ASSERT compiles out in release, but
      // a failed unmap leaves a leaked view with no recovery path.
