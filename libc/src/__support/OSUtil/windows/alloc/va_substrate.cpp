...
SubSlotHandle ArenaPool::acquire_impl(SubstrateRegistry &registry,
  ...
  for (;;) {
    ...
    // reconciliation.
    ArenaHeader *a = g_substrate_domain.read(active_, /*index=*/0,
                                             /*parent=*/nullptr);
    if (!a) {
...
// stamps the anchor's birth_epoch, links the node onto the calling
...
ArenaHeader *ArenaPool::reserve_new_arena(SubstrateRegistry &registry,
  ...
  // Crystalline-stamp non-permanent arenas. init_node sets birth_epoch
  ...
  // their birth_epoch is unread, and skipping init_node also avoids
...
void ArenaPool::initialize_arena_header(ArenaHeader *hdr, uintptr_t base,
  ...
  // CrystallineNode fields (next/slot/birth_epoch union, batch_link,
  // refs/batch_next union) are also zeroed by the memset — that is
  // the valid "freshly allocated, never retired" state. `init_node`
  // (called from the non-permanent reserve path) stamps birth_epoch
