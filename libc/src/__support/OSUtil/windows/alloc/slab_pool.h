...
struct alignas(64) SlabHeaderT
  ...
  // -- Cache line 0: inherited CrystallineNode + pad --
  //
  // The 24 bytes inherited from CrystallineNode (next/slot/birth_epoch
  // union, batch_link, refs/batch_next union) live at offset 0..23 and
  ...
  // time, not allocation time). The 40-byte pad pushes owner-mutable
  // fields onto cache line 1, so Crystalline's post-retire writes
  // (chain links, refcount adjustments) cannot collide with owner-side
  // writes still in flight on a slab that has just been observed by a
  // concurrent reader. Mirrors the layout choice made by ArenaHeader
  // (va_substrate.h:294) — same rationale.
  uint8_t _pad_post_crystalline[40];
...
// entirely.)
static_assert(sizeof(::LIBC_NAMESPACE::concurrent::CrystallineNode) == 24,
              "CrystallineNode size assumption — the 40-byte _pad_post_"
              "crystalline in SlabHeaderT depends on this. If "
              "CrystallineNode grows, shrink the pad to keep cache "
              "line 0 = 64 bytes.");
...
private:
  ...
  // reserve_slot fast-path (epoch unchanged) hits ~8× more often than
...
public:
  ...
  /// every slab whose retire was published in the current epoch. Cross-
  ...
  /// the dereference is in flight. Drops via a second reserve_slot which
  /// advances the slot's epoch (cheaper than `clear_all`'s full walk).
  LIBC_INLINE void retire_reservation_acquire() {
    slab_retire_domain_.reserve_slot(/*ptr_unused=*/nullptr,
                                     /*index=*/0,
                                     /*parent=*/nullptr);
  }
  LIBC_INLINE void retire_reservation_drop() {
    slab_retire_domain_.reserve_slot(nullptr, /*index=*/0, nullptr);
  }
  ...
  LIBC_INLINE void init(size_t slot_size, size_t slot_align = sizeof(void *),
    ...
    // retire/read/reserve_slot on this domain. Tier A bring-up is
