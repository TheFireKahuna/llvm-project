...
// DLL paths are short. MAX_PATH + NUL covers all practical cases.
inline constexpr int DLOPEN_PATH_MAX = 261;
...
intptr_t dlopen(const char *path, int mode) {
  ...
  // Convert UTF-8 path to wide string for NT loader APIs.
  WCHAR wpath[DLOPEN_PATH_MAX];
  int wlen = windows::utf8_to_wide(path, wpath, DLOPEN_PATH_MAX);
  if (wlen <= 0)
    return -EINVAL;

  UNICODE_STRING dll_name;
  ::RtlInitUnicodeString(&dll_name, wpath);

  // RTLD_NOLOAD: check if already loaded without incrementing the ref count.
  if (mode & RTLD_NOLOAD) {
    PVOID handle = nullptr;
    NTSTATUS st = ::LdrGetDllHandleByName(&dll_name, nullptr, &handle);
    if (NT_ERROR(st))
  ...
  PVOID handle = nullptr;
  NTSTATUS st = ::LdrLoadDll(nullptr, nullptr, &dll_name, &handle);
  if (NT_ERROR(st))
    return windows_util::ntstatus_to_kerr(st);
