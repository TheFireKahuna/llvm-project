...
LIBC_INLINE int chmod_impl(HANDLE h, mode_t mode) {
  ...
  HANDLE dacl_handle = nullptr; // Non-null if we reopened for WRITE_DAC.
  ...
  if (acl_size > 0) {
    ...
    if (status == STATUS_ACCESS_DENIED) {
      ...
      dacl_handle = reopen_with_access(h, WRITE_DAC);
      if (dacl_handle) {
        ::NtSetSecurityObject(dacl_handle, DACL_SECURITY_INFORMATION, &sd);
      }
    }
...
LIBC_INLINE int chown_impl(HANDLE h, uid_t uid, gid_t gid) {
  ...
  if (status == STATUS_ACCESS_DENIED) {
    work_handle = reopen_with_access(
...
LIBC_INLINE bool ensure_secure_ipc_dir(WCHAR *nt_path, size_t nt_path_len) {
  ...
  st = ::NtQueryInformationFile(dir_h, &basic_iosb, &basic, sizeof(basic),
                                 FileBasicInformation);
  if (NT_SUCCESS(st) &&
  ...
  if (safe && iosb.Information != FILE_CREATED_RESULT) {
    ...
    st = ::NtQuerySecurityObject(
        dir_h, OWNER_SECURITY_INFORMATION,
