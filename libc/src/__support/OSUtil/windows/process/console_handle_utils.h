...
LIBC_INLINE NTSTATUS build_conhost_image_path(WCHAR *buffer, size_t capacity,
  ...
  windows::WStringStream ss(cpp::span<WCHAR>(buffer, capacity - 1));
  ss << u"\\??\\" << windows_util::shared_user_data()->NtSystemRoot
     << u"\\System32\\conhost.exe";
  if (ss.overflow())
...
// Duplicate a handle with OBJ_INHERIT set, adopting the result into a
// ScopedNtHandle. On failure dst is left untouched.
LIBC_INLINE NTSTATUS duplicate_inherited_into(HANDLE source,
                                              windows::ScopedNtHandle &dst) {
  HANDLE raw = nullptr;
  NTSTATUS status = duplicate_inherited(source, &raw);
