...
NTSTATUS open_profile_list(HANDLE *key_out) {
  windows::nt_wstring_view key_name(PROFILE_LIST_KEY);
...
NTSTATUS open_subkey(HANDLE parent, const WCHAR *name, ULONG name_bytes,
                     HANDLE *key_out) {
  windows::nt_wstring_view key_name(name, name_bytes / sizeof(WCHAR));

  auto oa = windows::named_internal_oa(&key_name, parent);
...
size_t query_profile_path(HANDLE subkey, WCHAR *dst, size_t dst_cap) {
  ...
  NTSTATUS status = ::NtQueryValueKey(subkey, value_name.unicode_string(),
  ...
  __builtin_memcpy(dst, wide_data, wide_len * sizeof(WCHAR));
  dst[wide_len] = u'\0';
  return wide_len;
}
...
size_t uid_to_wellknown_sid_string(uid_t uid, WCHAR *dst, size_t dst_cap) {
  windows::WStringStream ss(cpp::span<WCHAR>(dst, dst_cap - 1));
  ss << u"S-1-5-" << static_cast<unsigned int>(uid);
  if (ss.overflow())
    return 0;
  ss.null_terminate();
  return ss.str().size();
...
size_t lookup_profile_by_sid_string(HANDLE profile_list_key,
  ...
  size_t result = query_profile_path(subkey.get(), path_buf, path_cap);
  return result;
}
...
// BufWriter is shared with group_ops.cpp — see buf_writer.h.
...
bool wide_eq_utf8_icase(const WCHAR *wide, size_t wide_len, const char *utf8) {
  ...
  return internal::string_util::ascii_streq_icase(converted, utf8);
}
...
int fill_passwd_uid(uid_t uid, struct passwd *pwd, char *buf, size_t buflen) {
  windows::ScopedNtHandle profile_list;
  NTSTATUS status = open_profile_list(profile_list.put());
  if (!NT_SUCCESS(status))
    return ENOENT;
  ...
  if (sid_len > 0)
    path_len = lookup_profile_by_sid_string(profile_list.get(), sid_str, sid_len,
                                            path_buf, 260);
  ...
  if (path_len == 0) {
    ...
    if (sid_len > 0)
      path_len = lookup_profile_by_sid_string(profile_list.get(), sid_str,
                                              sid_len, path_buf, 260);
  }
  ...
  if (path_len == 0) {
    ...
    for (ULONG index = 0;; ++index) {
      status =
          ::NtEnumerateKey(profile_list.get(), index, KeyBasicInformation,
                           key_buf, sizeof(key_buf), &result_len);
      if (!NT_SUCCESS(status))
        break;
      ...
      if (candidate_uid == uid) {
        path_len = lookup_profile_by_sid_string(
            profile_list.get(), info->Name, name_wchars, path_buf, 260);
        break;
      }
    }
  }
...
int fill_passwd_name(const char *name, struct passwd *pwd, char *buf,
  ...
  for (ULONG index = 0;; ++index) {
    ...
    size_t path_len = lookup_profile_by_sid_string(
        profile_list.get(), info->Name, sid_wchars, path_buf, 260);
    if (path_len == 0)
      continue;
