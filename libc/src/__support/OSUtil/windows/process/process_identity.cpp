...
void identity_init() {
  ...
  alignas(8) UCHAR owner_buf[128];
  ULONG needed = 0;
  NTSTATUS status = ::NtQueryInformationToken(
      token, TokenUser, owner_buf, sizeof(owner_buf), &needed);
  uid_t uid = 0;
  if (NT_SUCCESS(status)) {
    auto *tu = reinterpret_cast<TOKEN_USER *>(owner_buf);
    uid = internal::sid_to_uid(tu->User.Sid);
  }

  // Query primary group SID -> real_gid.
  alignas(8) UCHAR group_buf[128];
  status = ::NtQueryInformationToken(
      token, TokenPrimaryGroup, group_buf, sizeof(group_buf), &needed);
  gid_t gid = 0;
  if (NT_SUCCESS(status)) {
    auto *tpg = reinterpret_cast<TOKEN_PRIMARY_GROUP *>(group_buf);
...
NTSTATUS acquire_token_for_uid(uid_t uid, HANDLE *out_token) {
  *out_token = nullptr;
  ...
  s4u.Flags = 0;
  s4u.UserPrincipalName.Buffer = user_buf;
  s4u.UserPrincipalName.Length = static_cast<USHORT>(user_len * sizeof(WCHAR));
  s4u.UserPrincipalName.MaximumLength =
      static_cast<USHORT>((user_len + 1) * sizeof(WCHAR));
  s4u.DomainName.Buffer = nullptr;
  s4u.DomainName.Length = 0;
  s4u.DomainName.MaximumLength = 0;
  ...
  status = ::LsaLogonUser(lsa, &origin_str, LOGON_TYPE_NETWORK, auth_package,
                           ...
                           &profile_len, &logon_id, &new_token, &quotas,
                           &sub_status);
  ...
  status = ::NtDuplicateToken(new_token,
...
int set_exec_ids(uid_t file_uid, gid_t file_gid, bool setuid_bit,
  ...
  if (setuid_bit && file_uid != g_pcb.identity.eff_uid.load(
    ...
    HANDLE old_token = g_pcb.identity.impersonation_token.exchange(
...
int LIBC_NAMESPACE::internal::identity_startup_init() {
  ...
  // Last Zone 0 write: parent PID via ProcessBasicInformation.
  // Must complete before pcb_seal_readonly().
