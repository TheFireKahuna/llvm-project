...
LIBC_INLINE DWORD desired_input_mode(const struct termios &) {
  // Leave ENABLE_LINE_INPUT and ENABLE_PROCESSED_INPUT on so InputBuffer
  // retains the conhost-default buffering semantics the VtInputThread was
  // tested against. The libc tty discipline still does its own canonical
  // editing/echo/signal generation in user space — the bits here only
  // affect conhost's internal bookkeeping, not what our slave reads.
  // Also include ENABLE_WINDOW_INPUT so resize events (SIGWINCH) still
  // propagate through the input queue.
  return condrv::ENABLE_PROCESSED_INPUT | condrv::ENABLE_LINE_INPUT |
         condrv::ENABLE_ECHO_INPUT | condrv::ENABLE_MOUSE_INPUT |
         condrv::ENABLE_WINDOW_INPUT;
}
