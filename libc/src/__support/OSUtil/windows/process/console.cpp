...
size_t cstr_length(const WCHAR *src) {
  if (!src)
    return 0;
...
build_default_conhost_strings(const SessionOptions &options,
  ...
  if (options.server_image_path) {
    ...
    size_t root_len = cstr_length(sud->NtSystemRoot);
    size_t prefix_len = sizeof(NT_PATH_PREFIX) / sizeof(WCHAR) - 1;
    size_t suffix_len = sizeof(DEFAULT_CONHOST_SUFFIX) / sizeof(WCHAR) - 1;
    size_t total_len = prefix_len + root_len + suffix_len;
    if (total_len + 1 > image_capacity)
      return STATUS_BUFFER_TOO_SMALL;

    size_t pos = 0;
    for (size_t i = 0; i < prefix_len; ++i)
      image_buffer[pos++] = NT_PATH_PREFIX[i];
    for (size_t i = 0; i < root_len; ++i)
      image_buffer[pos++] = sud->NtSystemRoot[i];
    for (size_t i = 0; i < suffix_len; ++i)
      image_buffer[pos++] = DEFAULT_CONHOST_SUFFIX[i];
    image_buffer[pos] = 0;
  ...
  size_t image_len = cstr_length(image_path->Buffer);
  size_t args_len = sizeof(DEFAULT_CONHOST_ARGUMENTS) / sizeof(WCHAR) - 1;
  if (image_len + args_len + 1 > command_capacity)
    return STATUS_BUFFER_TOO_SMALL;

  size_t pos = 0;
  for (size_t i = 0; i < image_len; ++i)
    command_buffer[pos++] = image_path->Buffer[i];
  for (size_t i = 0; i < args_len; ++i)
    command_buffer[pos++] = DEFAULT_CONHOST_ARGUMENTS[i];
...
HANDLE current_reference() { return load_current_console_reference(); }
...
// Phase 8 startup init — sync console state.
// Called by __libc_dll_init() after stdio wiring.
int LIBC_NAMESPACE::internal::console_startup_init() {
