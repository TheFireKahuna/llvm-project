...
public:
  ...
  LIBC_INLINE ~TreeStateLock() {
    if (locked_)
      state().lock.unlock();
  }
  TreeStateLock(const TreeStateLock &) = delete;
  TreeStateLock &operator=(const TreeStateLock &) = delete;

  /// Release the lock early; destructor becomes a no-op.
...
size_t format_namespace_name(WCHAR *buffer, size_t cap) {
  windows::WStringStream ss(cpp::span<WCHAR>(buffer, cap - 1));
  ss << NAMESPACE_PREFIX;
  for (size_t i = 0; i < 16; ++i)
    ss.write_int<radix::Hex::Uppercase::WithWidth<2>>(state().tree_nonce[i]);
  ss.null_terminate();
  return ss.str().size();
}

void init_named_attributes(OBJECT_ATTRIBUTES *oa, windows::nt_wstring_view *name,
                           HANDLE root, WCHAR *buffer, size_t length) {
  *name = windows::nt_wstring_view(buffer, length);
  *oa = windows::named_internal_oa(name, root);
}
...
POBJECT_BOUNDARY_DESCRIPTOR create_boundary_descriptor_locked() {
  ...
  windows::nt_wstring_view ns_name(ns_buf, ns_len);

  POBJECT_BOUNDARY_DESCRIPTOR boundary =
      ::RtlCreateBoundaryDescriptor(ns_name.unicode_string(), BOUNDARY_DESCRIPTOR_FLAG_NONE);
  if (!boundary)
    return nullptr;
...
NTSTATUS create_named_section_locked(const WCHAR *leaf, SIZE_T size,
  ...
  *section_out = nullptr;
  ...
  windows::nt_wstring_view name(leaf);
  OBJECT_ATTRIBUTES oa =
      windows::named_internal_oa(&name, state().namespace_handle);
...
int create_shared_state_locked(uint32_t id, HANDLE *state_lock,
  ...
  windows::ScopedNtHandle mutant;
  NTSTATUS status =
      ::NtCreateMutant(mutant.put(), MUTANT_ALL_ACCESS, &mutant_oa, FALSE);
  if (!NT_SUCCESS(status))
  ...
  windows::SectionView view =
      windows::SectionView::map_anywhere(section, PAGE_READWRITE,
                                         sizeof(PtySharedState), {}, &status);
  if (!view)
    return windows_util::ntstatus_to_errno(status);

  auto *shared = view.as<PtySharedState>();
  __builtin_memset(shared, 0, sizeof(PtySharedState));
  shared->magic = PTY_SHARED_STATE_MAGIC;
  shared->version = PTY_SHARED_STATE_VERSION;
  shared->id = id;

  *state_lock = mutant.release();
  *state_section = section.release();
  *state_view = static_cast<PtySharedState *>(view.detach());
  return 0;
}
...
int validate_shared_state_view(HANDLE state_section, uint32_t expected_id,
  ...
  windows::SectionView view = windows::SectionView::map_anywhere(
      section_ref, PAGE_READWRITE, sizeof(PtySharedState), {}, &status);
  if (!view) {
    view = windows::SectionView::map_anywhere(
        section_ref, PAGE_READONLY, sizeof(PtySharedState), {}, &status);
  }
  if (!view)
    return windows_util::ntstatus_to_errno(status);

  auto *shared = view.as<PtySharedState>();
  if (shared->magic != PTY_SHARED_STATE_MAGIC ||
      (shared->version != PTY_SHARED_STATE_VERSION && shared->version != 2) ||
      (expected_id != 0 && shared->id != expected_id)) {
    return EIO;
  }
  *out = static_cast<windows::SectionView &&>(view);
  return 0;
}
...
NTSTATUS ensure_tree_job_locked() {
  ...
  windows::nt_wstring_view name(TREE_JOB_NAME);
  OBJECT_ATTRIBUTES oa =
      windows::named_internal_oa(&name, state().namespace_handle);
...
// Common ALPC port attributes; |flags| captures the single divergence
// (ensure_service_locked passes ALPC_PORFLG_ALLOW_LPC_REQUESTS).
ALPC_PORT_ATTRIBUTES make_alpc_port_attributes(ULONG flags = 0) {
...
// Populate the PORT_MESSAGE header and common PtyStateRequest fields.
// The caller fills |req.payload| for the specific op.
void fill_state_request_header(PtyStateRequest &req, PtyStateOp op,
                               uint32_t pty_id) {
...
NTSTATUS alloc_state_view_locked(HANDLE *section_out, PtyAllocState **view_out) {
  ...
  *view_out = nullptr;
  ...
  windows::SectionView view = windows::SectionView::map_anywhere(
      section, PAGE_READWRITE, sizeof(PtyAllocState), {}, &status);
  if (!view)
    return status;

  auto *alloc_state = view.as<PtyAllocState>();
  ...
  state().alloc_state_view = view.detach();
  *section_out = state().alloc_section;
  *view_out = static_cast<PtyAllocState *>(state().alloc_state_view);
  return STATUS_SUCCESS;
}
...
size_t format_service_leaf_name(WCHAR *buffer, size_t cap, DWORD pid,
  ...
  windows::WStringStream ss(cpp::span<WCHAR>(buffer, cap - 1));
  ss << SERVICE_PREFIX << static_cast<unsigned int>(pid) << u'-';
...
NTSTATUS alpc_transact_with_service(DWORD owner_pid, uint64_t owner_create_time,
  ...
  ALPC_PORT_ATTRIBUTES port_attrs = make_alpc_port_attributes();

  *receive_length = message_buf_size;
  ...
  NTSTATUS status = ::NtAlpcConnectPortEx(
      ...
      nullptr, reinterpret_cast<PPORT_MESSAGE>(message_buf), receive_length,
...
OwnedHandles duplicate_owned_handles_for_pid(const OwnedHandles &source,
  ...
  OwnedHandles result = {};
  auto cleanup =
      cpp::make_scope_guard([&] { close_owned_handles(result); });
...
void handle_state_request(HANDLE server_port, PPORT_MESSAGE request) {
  ...
  if (valid) {
    ...
    if (mutant && section) {
      NTSTATUS map_status = STATUS_SUCCESS;
      auto section_ref = windows::SectionHandle::borrow(section);
      windows::SectionView view = windows::SectionView::map_anywhere(
          section_ref, PAGE_READWRITE, sizeof(PtySharedState), {}, &map_status);
      if (view) {
        auto *shared = view.as<PtySharedState>();
        ...
        if (NT_SUCCESS(lock_status)) {
          ...
          case PTY_STATE_OP_SET_ATTR:
            shared->attrs = state_req->payload.attrs;
            reply.status = STATUS_SUCCESS;
            break;
          case PTY_STATE_OP_SET_FOREGROUND_PGRP:
            if (state_req->payload.pgid > 0) {
              shared->foreground_pgrp = state_req->payload.pgid;
              reply.status = STATUS_SUCCESS;
            }
            break;
  ...
  ALPC_PORT_ATTRIBUTES attrs = make_alpc_port_attributes();
...
NTSTATUS ensure_service_locked() {
  ...
  ALPC_PORT_ATTRIBUTES attrs =
      make_alpc_port_attributes(ALPC_PORFLG_ALLOW_LPC_REQUESTS);

  HANDLE port = nullptr;
  status = ::NtAlpcCreatePort(&port, &oa, &attrs);
  if (!NT_SUCCESS(status))
    return status;
  auto close_port = cpp::make_scope_guard([&] { ::NtClose(port); });

  auto *start = static_cast<ServiceThreadStartContext *>(
      page_alloc(sizeof(ServiceThreadStartContext)));
  if (!start)
    return STATUS_NO_MEMORY;
  auto free_start = cpp::make_scope_guard([&] { page_free(start); });
  start->port = port;
...
ErrorOr<OwnedHandles> connect_to_service_locked(uint32_t id, DWORD owner_pid,
  ...
  LARGE_INTEGER timeout = {};
  timeout.QuadPart = -2000000LL; // 200ms
  SIZE_T receive_length = 0;
  status = alpc_transact_with_service(owner_pid, owner_create_time, &message,
                                      sizeof(message), &receive_length, timeout);
  if (!NT_SUCCESS(status)) {
    if (retryable)
      *retryable = true;
    return Error(windows_util::ntstatus_to_errno(status));
...
ErrorOr<OwnedHandles> request_handles_locked(uint32_t id, DWORD owner_pid,
  ...
  {
    TreeStateLock guard;
    status = open_join_lock_locked(&join_lock);
  }
  if (!NT_SUCCESS(status))
    return Error(windows_util::ntstatus_to_errno(status));
  ...
  for (int attempt = 0; attempt < 2; ++attempt) {
    ...
    ErrorOr<OwnedHandles> result = Error(EIO);
    {
      TreeStateLock guard;
      result =
...
void close_current_state_locked() {
  unmap_raw_view(&state().current_state_view);
...
void close_allocator_locked() {
  unmap_raw_view(&state().alloc_state_view);
...
int map_current_state_locked(const CurrentAttachment &attachment) {
  ...
  windows::SectionView view;
  int err = validate_shared_state_view(attachment.state_section,
                                       attachment.pty_id, &view);
  if (err != 0)
    return err;

  state().current_state_lock = attachment.state_lock;
  state().current_state_section = attachment.state_section;
  state().current_state_view = view.detach();
...
void fini() {
  auto &tree = state();
  {
    ...
    if (tree.boundary_descriptor) {
      ::RtlDeleteBoundaryDescriptor(reinterpret_cast<POBJECT_BOUNDARY_DESCRIPTOR>(
          tree.boundary_descriptor));
...
void fork_reinit() {
  ...
  unmap_raw_view(&tree.current_state_view);
...
ErrorOr<uint32_t> allocate_pty_id() {
  ...
  {
    TreeStateLock guard;
    lock_status = open_alloc_lock_locked(&alloc_lock);
    if (NT_SUCCESS(lock_status))
...
ErrorOr<int> create_shared_state(uint32_t id, HANDLE *state_lock,
  ...
  *state_view = nullptr;
  ...
  {
    TreeStateLock guard;
    err = create_shared_state_locked(id, state_lock, state_section, state_view);
  }
  if (err != 0)
    return Error(err);
  return 0;
...
ErrorOr<OwnedHandles> join_handles(uint32_t id) {
  ...
  for (DWORD i = 0; i < count; ++i) {
    ...
    uint16_t flags = view.as<PtySharedState>()->flags;
    if (locked)
...
uint32_t current_attached_pty_id() {
  init();
  ::LIBC_NAMESPACE::internal::vt_pty::ensure_adoption();
  return current_attached_pty_id_unchecked();
}
...
HANDLE take_inherited_reference() {
  init();
  TreeStateLock guard;
  HANDLE reference = state().inherited_reference;
  state().inherited_reference = nullptr;
  return reference;
}
...
uint16_t inherited_attach_flags() {
  init();
  TreeStateLock guard;
  return state().inherited_attach_flags;
}
...
int duplicate_current_attachment(CurrentAttachment *attachment, bool inherit) {
  ...
  *attachment = {};
  HANDLE source_lock = nullptr;
  HANDLE source_section = nullptr;
  {
    TreeStateLock guard;
    attachment->pty_id =
        state().current_attached_pty_id.load(cpp::MemoryOrder::ACQUIRE);
    source_lock = state().current_state_lock;
    source_section = state().current_state_section;
  }
...
int set_current_attached_pty(const CurrentAttachment &attachment) {
  ...
  {
    TreeStateLock guard;
    err = map_current_state_locked(local);
  }
  if (err != 0) {
...
int with_current_state_seqlock(Fn &&fn) {
  ...
  {
    TreeStateLock guard;
    view = static_cast<const PtySharedState *>(state().current_state_view);
  }
  if (!view)
    return -ENOTTY;
...
int send_state_request_to_owner(const PtyStateRequest &req, uint64_t owner_pid,
  ...
  SIZE_T receive_length = 0;
  NTSTATUS status = alpc_transact_with_service(
      static_cast<DWORD>(owner_pid), owner_create_time, &message,
      sizeof(message), &receive_length, timeout);
  if (!NT_SUCCESS(status))
    return -windows_util::ntstatus_to_errno(status);
...
int send_current_state_request(const PtyStateRequest &req) {
  ...
  uint64_t owner_create_time;
  {
    TreeStateLock guard;
    auto *view =
        static_cast<const PtySharedState *>(state().current_state_view);
    if (!view)
      return -ENOTTY;
...
int query_state_owner(HANDLE state_section, uint32_t expected_id,
  ...
  *owner_create_time = shared->owner_create_time;
  return 0;
}
...
int current_set_attr(const struct termios *attrs) {
  ...
  PtyStateRequest req = {};
  fill_state_request_header(req, PTY_STATE_OP_SET_ATTR, pty_id);
...
int dispatch_state_mutation(uint32_t pty_id, HANDLE state_section,
  ...
  PtyStateRequest req = {};
  fill_state_request_header(req, op, pty_id);
  fill_payload(req.payload);
  return send_state_request_to_owner(req, owner_pid, owner_create_time);
}
...
int set_attr(uint32_t pty_id, HANDLE state_section, const struct termios *attrs) {
  if (pty_id == 0 || !state_section || !attrs)
    return -EINVAL;
  return dispatch_state_mutation(
      pty_id, state_section, PTY_STATE_OP_SET_ATTR,
      [&](PtySharedState &shared) { shared.attrs = *attrs; },
      [&](auto &payload) { payload.attrs = *attrs; });
}
...
int current_set_foreground_pgrp(pid_t pgid) {
  ...
  // Slave path: send ALPC request to master.
  PtyStateRequest req = {};
  fill_state_request_header(req, PTY_STATE_OP_SET_FOREGROUND_PGRP, pty_id);
...
int set_foreground_pgrp(uint32_t pty_id, HANDLE state_section, pid_t pgid) {
  if (pty_id == 0 || !state_section || pgid <= 0)
    return -EINVAL;
  return dispatch_state_mutation(
      pty_id, state_section, PTY_STATE_OP_SET_FOREGROUND_PGRP,
      [&](PtySharedState &shared) { shared.foreground_pgrp = pgid; },
      [&](auto &payload) { payload.pgid = pgid; });
}
...
int set_session_controller(uint32_t pty_id, HANDLE state_section, pid_t sid,
  ...
  if (pty_id == 0 || !state_section || sid <= 0 || pgid <= 0 ||
    ...
    return -EINVAL;
  return dispatch_state_mutation(
      pty_id, state_section, PTY_STATE_OP_SET_SESSION_CONTROLLER,
      [&](PtySharedState &shared) {
        shared.controlling_sid = sid;
        shared.foreground_pgrp = pgid;
        shared.controller_pid = controller_pid;
        shared.controller_create_time = controller_create_time;
      },
      [&](auto &payload) {
        payload.controller.controller_create_time = controller_create_time;
        payload.controller.controlling_sid = sid;
        payload.controller.foreground_pgrp = pgid;
        payload.controller.controller_pid = controller_pid;
        payload.controller.reserved = 0;
      });
}
...
LIBC_REGISTER_FORK_REINIT(pty_tree,
                          ::LIBC_NAMESPACE::internal::kForkPrioPtyTree,
                          &::LIBC_NAMESPACE::internal::pty_tree_fork_reinit)
