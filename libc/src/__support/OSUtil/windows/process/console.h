...
// MS-ABI ctrl dispatcher type. `decltype` on an unreferenced external
// prototype — LIBC_MSABI attaches to the declaration, not to a type
// alias (trailing-alias form trips `-Wgcc-compat`).
LIBC_MSABI DWORD __ctrl_dispatch_type_source(ULONG_PTR raw_param);
using CtrlDispatchFn = decltype(&__ctrl_dispatch_type_source);
