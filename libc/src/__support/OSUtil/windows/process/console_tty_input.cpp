...
int try_read_input_record(HANDLE input_handle,
  ...
  // Records in conhost's gci.pInputBuffer are always stored as UNICODE. A
  // read with unicode=false asks conhost to re-encode to the input codepage,
  // but in the current installed conhost this silently returns count=0 for
  // ASCII printable records (verified via pty_probe peek). Read as UNICODE
  // here and let process_input_record handle narrow vs wide translation on
  // the UnicodeChar field (which is populated whether the event came from
  // CharToKeyEvents or our own write_console_input_on inject).
  NTSTATUS status = condrv::read_console_input_ex(
      input_handle, record, 1, &record_count, condrv::CONSOLE_READ_NOWAIT,
      /*unicode=*/true, io_target);
...
ssize_t read_canonical(ConsoleTtyState &state,
  ...
  for (;;) {
    ...
    condrv::CONSOLE_INPUT_RECORD record = {};
    int record_result = try_read_input_record(input_handle, &record, io_target);
    if (record_result < 0)
...
ssize_t read_noncanonical(ConsoleTtyState &state,
  ...
  for (;;) {
    ...
    // VMIN=0 VTIME=0 semantics: return whatever is immediately available
    // without blocking. "Immediately available" must include any record
    // already queued in conhost's pInputBuffer, not just bytes buffered
    // inside ConsoleTtyState. Fall through to the CONSOLE_READ_NOWAIT
    // fetch below; if that returns zero records we'll then return per
    // the nonblocking/blocking conventions.
