...
int LIBC_NAMESPACE::internal::mitigation_policy_startup_init() {
  ...
  // RedirectionTrust (Win11+): the loader refuses to follow filesystem
  // junctions / symlinks across trust boundaries during DLL resolution.
  // Non-fatal — older Win10 ignores it.
  info = {};
  info.Policy = ProcessRedirectionTrustPolicyClass;
  info.RedirectionTrustPolicy.EnforceRedirectionTrust = 1;
  (void)set_mitigation_policy(info);

  // TODO: ProcessFontDisablePolicyClass — blocks GDI font parsing in
  // the process. Struct: { ULONG DisableNonSystemFonts : 1; }. Useful
  // hardening against the recurring CVE class around TrueType /
  // OpenType parsing in win32k. Not wired here because it can break
  // user code that legitimately loads custom fonts (text rendering
  // tools, document viewers); revisit when we ship a server-only profile.
  //
  // TODO: ProcessPayloadRestrictionPolicyClass — Export Address Filter
  // (EAF) and Import Address Filter (IAF), heap-spray and ROP gadget
  // import-resolution interception. Struct includes EnableExportAddressFilter,
  // EnableImportAddressFilter, EnableRopStackPivot, EnableRopCallerCheck,
  // EnableRopSimExec. Powerful but commonly tripped by AV/EDR injection
  // and JIT engines (V8, .NET); revisit alongside an opt-in compatibility
  // matrix.
