...
// space snapshot (RtlCloneUserProcess, which wraps NtCreateUserProcess with
// the cloning flag) and BEFORE user pthread_atfork prepare handlers.
//
// Two concerns:
//
...
//
// Intentionally non-RAII: the lock span crosses the boundary between two
// separate public APIs (libc_fork_quiesce -> ... -> libc_fork_resume).
// MmapLockWriterGuard and equivalents are non-movable and cannot bridge
// that boundary without escaping into static state, which would defeat
// scope-bound ownership.  The paired acquire/release functions are the
...
// Both libc_fork_quiesce and libc_fork_resume live in this TU; one anchor
// covers both.
LIBC_ANCHOR_DEFINE(fork, quiesce);
