...
#include "src/__support/OSUtil/windows/memory/numa_ops.h"
...
///
/// Uses bulk VA walk (NtPssCaptureVaSpaceBulk via RegionWalker) for the
...
int verify_numa_placement(HANDLE process, char *start,
  ...
  if (!ws) return -ENOMEM;
  windows::RegionWalker walk(start, range_size, ws.data(), ws.size());
...
long rebind_region(HANDLE process, char *start, SIZE_T size,
  ...
  if (is_interleave) {
    ...
    st = entry.spec.map_into_reserve(start, size);
    ok = NT_SUCCESS(st);

    if (ok) {
      // Restore COW pages on top of the SEC_RESERVE remap.
      guard.cow().restore(base_val, guard.records(), guard.record_count());

      windows::ViewSpec interleave_spec = entry.spec;
      interleave_spec.flags = entry.spec.flags |
                              windows::VM_FLAG_SEC_RESERVE |
                              windows::VM_FLAG_NUMA_INTERLEAVE;
      guard.commit(start, size, interleave_spec);
    ...
    st = entry.spec.map_into_ex(start, size, &param, 1);

    if (NT_ERROR(st)) {
      // Fallback: remap without NUMA to avoid losing the mapping.
      st = entry.spec.map_into(start, size);
    }
    ok = NT_SUCCESS(st);

    if (ok) {
      // Restore COW pages + replay protections.
      guard.cow().restore(base_val, guard.records(), guard.record_count());
      if (guard.record_count() > 0)
        windows::replay_protections(process, base_val, entry.spec.prot,
                                    guard.records(), guard.record_count());

      guard.commit(start, size, entry.spec);
    }
  }
...
intptr_t set_mempolicy(int mode, const unsigned long *nodemask,
  ...
  ULONG ret_len = 0;
  if (NT_ERROR(::NtQuerySystemInformation(SystemNumaProcessorMap,
                                          &highest_node, sizeof(highest_node),
                                          &ret_len))) {
    return -EIO;
  }
...
intptr_t get_mempolicy(int *mode, unsigned long *nodemask, unsigned long maxnode,
  ...
  if (flags & MPOL_F_ADDR) {
    ...
    if (!windows::query_region(addr, &info)) {
      return -EFAULT;
    }
...
intptr_t mbind(void *addr, unsigned long len, int mode,
  ...
  if (nodemask && maxnode > 0) {
    ...
    // Clamp to nodes that actually exist.
    ULONG highest_node = 0;
    ULONG ret_len = 0;
    if (NT_SUCCESS(::NtQuerySystemInformation(SystemNumaProcessorMap,
                                              &highest_node,
                                              sizeof(highest_node),
                                              &ret_len)) &&
        highest_node < 63)
