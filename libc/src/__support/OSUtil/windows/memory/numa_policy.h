...
/// Check if the system has multiple NUMA nodes. Cached on first call.
LIBC_INLINE bool is_numa_system() {
  static bool result = []() {
    ULONG highest = 0;
    ULONG ret_len = 0;
    NTSTATUS st = ::NtQuerySystemInformation(SystemNumaProcessorMap,
                                             &highest, sizeof(highest),
                                             &ret_len);
    return NT_SUCCESS(st) && ret_len >= sizeof(ULONG) && highest > 0;
  }();
  return result;
}
