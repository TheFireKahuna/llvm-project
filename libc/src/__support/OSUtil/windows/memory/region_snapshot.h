...
//       5. replace_placeholder_commit dirty clusters → private READWRITE
...
LIBC_INLINE int snapshot_regions(uintptr_t start, uintptr_t end,
  ...
  if (!ws) return -1;
  RegionWalker walk(reinterpret_cast<void *>(start), range_size, ws.data(),
                    ws.size());
  while (walk.next()) {
...
///   1. Write-watch (MEM_WRITE_WATCH regions): `write_watch_query_reset`
...
LIBC_INLINE CowSaveBuffer save_cow_pages(uintptr_t base,
  ...
  // MEM_WRITE_WATCH). If so, use the faster write_watch_query_reset path.
  bool use_write_watch = false;
  for (int i = 0; i < count; ++i) {
    if (records[i].is_cow) {
      uintptr_t first_cow = base + records[i].offset;
      MEMORY_REGION_INFORMATION mri;
      if (query_region_mri(reinterpret_cast<void *>(first_cow), mri))
  ...
  if (use_write_watch) {
    ...
    for (int r = 0; r < count; ++r) {
      ...
      SIZE_T found = write_watch_query_reset(
...
public:
  ...
  LIBC_INLINE bool prepare(uintptr_t base, const RegionRecord *records,
                           int count) {
...
LIBC_INLINE bool remap_with_cow_splits(
  ...
  auto commit_dirty = [&](uintptr_t start, SIZE_T size) -> bool {
    NTSTATUS st = replace_placeholder_commit(
  ...
  auto complete_run = [&](uintptr_t addr) -> bool {
    ...
    if (addr < frag_base + frag_size) {
      if (!split_placeholder(reinterpret_cast<void *>(run_start), run_size))
        return false;
    }
