...
#include "src/__support/OSUtil/windows/memory/anon_region_ops.h"
...
/// Partial committed: interior_release punches sub-range to MEM_FREE.
...
///                 POSIX munmap semantics) and re-mmap via vm_commit works.
///   * NORESERVE (demand-commit via VEH) / PROT_NONE (bare placeholder):
///       Full   — vm_release (MEM_RELEASE) on the view_base.
///       Partial — split_placeholder at the chunk bounds, release the
...
long unmap_anon_placeholder(char *chunk_start, char *chunk_end,
  ...
  if (is_full) {
    ...
    if (!windows::vm_release(region_base)) {
      bool republished =
  ...
  if (committed_flag) {
    ...
    if (!windows::decommit_private_range(chunk_start, sz))
      return -EINVAL;
    return 0;
  }
  ...
  if (cs > rb) {
    if (!windows::split_placeholder(region_base, cs - rb))
      return -EINVAL;
  }
  uintptr_t re = reinterpret_cast<uintptr_t>(region_end);
  if (ce < re) {
    if (!windows::split_placeholder(reinterpret_cast<void *>(cs), ce - cs))
      return -EINVAL;
  }
...
long full_unmap_view(HANDLE /*process*/, char *view_base) {
  ...
  if (NT_ERROR(st)) {
    ...
    // unmap_view (no placeholder preservation) returns the VA to
    // MEM_FREE; release_placeholder is a no-op-or-cleanup safety net for
    ...
    // refcount.
    NTSTATUS hard = windows::unmap_view(view_base);
    if (NT_ERROR(hard))
      windows::release_placeholder(view_base);
    if (entry.region_id != windows::memory::RegionPool::NONE)
      windows::memory::g_region_pool.release(entry.region_id);
