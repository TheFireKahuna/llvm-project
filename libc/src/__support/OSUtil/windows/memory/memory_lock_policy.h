...
struct OnfaultState {
  ...
  void read_unlock() {
    FutexValueType prev = rw_state.load(cpp::MemoryOrder::RELAXED);
    FutexValueType desired = prev - 2;
    while (!rw_state.compare_exchange_strong(prev, desired,
...
public:
  ...
  static LONG WINAPI onfault_veh(EXCEPTION_POINTERS *ep);
};
