...
/// `trim_anon_slot_for_hole` (re-publishes survivors) + `interior_release`
  ...
  // Phase 2 since alloc_fixed_free does not require it.
  {
    ...
    while (cur < end) {
      ...
      if (overlap_start <= region_base && overlap_end >= region_end) {
        ...
        // ref. Three steps — extract is the table mutation, vm_release the
        ...
        // table-side bookkeeping; interior_release owns the NT-side punch.
        ...
        if (!windows::interior_release(overlap_start, overlap_size))
          return -EINVAL;
      }
...
intptr_t alloc_large_pages(void *hint, SIZE_T size, int prot,
  ...
  if ((flags & MAP_FIXED) && hint) {
    ...
    if (!publish_view_commit(hint, reinterpret_cast<void *>(result), rounded,
      ...
      windows::unmap_view_preserve(reinterpret_cast<void *>(result));
      if (section)
        ::NtClose(section);
  ...
  if (!publish_view_register(reinterpret_cast<void *>(result), rounded, section,
    ...
    windows::unmap_view_preserve(reinterpret_cast<void *>(result));
    windows::release_placeholder(reinterpret_cast<void *>(result));
    if (section)
      ::NtClose(section);
    return -ENOMEM;
  }
...
intptr_t alloc_file_large_pages(void *hint, SIZE_T size, int prot,
  ...
  if ((flags & MAP_FIXED) && hint) {
    ...
    if (file_handle &&
      ...
      windows::unmap_view_preserve(reinterpret_cast<void *>(view));
      if (out_section)
        ::NtClose(out_section);
      return -ENOMEM;
    }
  ...
  auto view_guard = cpp::make_scope_guard([&] {
    windows::unmap_view(reinterpret_cast<void *>(view));
  });
...
intptr_t alloc_file_fixed(void *addr, SIZE_T size, int prot, int flags,
  ...
  if (file_handle &&
      NT_ERROR(::NtDuplicateObject(NtCurrentProcess(), file_handle,
                                   NtCurrentProcess(), &dup_file, 0, 0,
                                   DUPLICATE_SAME_ACCESS))) {
    windows::unmap_view_preserve(view_base);
    if (out_section)
      ::NtClose(out_section);
    return -ENOMEM;
  }
...
intptr_t alloc_fixed_free_commit(void *placeholder, SIZE_T size,
  ...
  if (prot == PROT_NONE) {
    ...
    if (!windows::publish_anon_placeholder_protnone(base, sz)) {
      windows::release_placeholder(base);
      return -ENOMEM;
    }
    return cpp::bit_cast<intptr_t>(base);
  }
...
intptr_t mmap(void *addr, size_t size, int prot, int flags, int fd,
  ...
  // --- File-backed mapping ---
  {
    ...
    if (noreplace_ph) {
      ...
      auto view_guard = cpp::make_scope_guard([&] {
        windows::unmap_view(reinterpret_cast<void *>(result));
      });
