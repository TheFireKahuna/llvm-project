...
int collect_view_slices(void *addr, SIZE_T total_size, ViewSliceEntry *out,
  ...
  // Bulk MBI via RegionWalker; snapshot reflects pre-mutation VA layout,
  // which is what we want — extract() touches the mapping table, not VA.
  auto ws = windows::byte_scratch(4096);
  if (!ws)
    return 0;
  windows::RegionWalker walk(addr, total_size, ws.data(), ws.size());
...
intptr_t move_section(void *old_addr, SIZE_T old_size,
  ...
  // flushes TLB entries for that range.
  {
    ...
    auto ws = windows::byte_scratch(4096);
    if (ws) {
      windows::RegionWalker walk(old_addr, old_size, ws.data(), ws.size());
      while (walk.next()) {
  ...
  if (!map_slices_into_placeholder(process, new_placeholder, slices,
    ...
    // tell us where each placeholder ends, even as release_placeholder runs.
    auto ws = windows::byte_scratch(4096);
    if (ws) {
      windows::RegionWalker walk(new_placeholder, new_size, ws.data(),
                                 ws.size());
      while (walk.next()) {
        windows::release_placeholder(walk.entry->BaseAddress);
      }
    }
  ...
  // ── Step 7: Unmap all old views ──
  {
    ...
    // to unmap, and unmap_view_preserve replaces views with placeholders
    ...
    if (ws) {
      ...
      while (walk.next()) {
        if (walk.entry->Type == MEM_MAPPED) {
          windows::unmap_view_preserve(walk.entry->AllocationBase);
        }
      }
    }
  ...
  if (dontunmap) {
    ...
    windows::vm_coalesce_placeholders(old_addr, old_size);
    ...
    if (ws) {
      windows::RegionWalker walk(old_addr, old_size, ws.data(), ws.size());
      while (walk.next()) {
        windows::release_placeholder(walk.entry->AllocationBase);
      }
    }
    // Scratch failure: cannot enumerate fragments — best-effort fall-through.
  }
  ...
  if (ext_section) {
    ...
    if (ext_region_id == windows::memory::RegionPool::NONE) {
      ...
      // unmap, and unmap_view replaces them with placeholders.
      auto ws = windows::byte_scratch(4096);
      if (ws) {
        windows::RegionWalker walk(new_placeholder, new_size, ws.data(),
                                   ws.size());
        while (walk.next()) {
          if (walk.entry->Type == MEM_MAPPED)
            windows::unmap_view(walk.entry->AllocationBase);
        }
      }
      // Scratch failure: cannot enumerate views — best-effort fall-through.
      release_view_slices(slices, slice_count);
...
intptr_t grow_section_in_place(void *addr, SIZE_T old_size,
  ...
  if (entry_file) {
    ...
    st = windows::unmap_view_preserve_transient(addr);
    if (NT_ERROR(st)) {
      // cow_ctx RAII handles cleanup.
      // ~ext_ph releases the extension placeholder.
      windows::g_mapping_table.abort_remap(addr, entry);
    ...
    st = windows::vm_coalesce_placeholders(addr, new_size);
    if (NT_ERROR(st)) {
    ...
    if (NT_ERROR(st)) {
      ...
      if (rec_count > 0)
        windows::replay_protections(process, addr_val, entry_prot,
                                    records.data(), rec_count);
      cow_ctx.restore(addr_val, records.data(), rec_count);
      windows::release_placeholder(ext_addr);
    ...
    if (!windows::g_mapping_table.commit_remap(
      ...
      windows::unmap_view_preserve(addr);
      windows::release_placeholder(addr);
    ...
    if (ext_region_id == windows::memory::RegionPool::NONE) {
      ...
      windows::unmap_view_preserve(ext_addr);
      ::NtClose(ext_section);
      windows::release_placeholder(ext_addr);
      windows::g_mapping_table.abort_remap(addr, entry);
      return 0;
...
intptr_t grow_bare_placeholder_in_place(void *addr, SIZE_T old_size,
  ...
  NTSTATUS st = windows::vm_coalesce_placeholders(addr, new_size);
  if (NT_ERROR(st)) {
...
intptr_t shrink_bare_placeholder(void *addr, SIZE_T new_size) {
  ...
  windows::release_placeholder(tail);
  return static_cast<intptr_t>(reinterpret_cast<uintptr_t>(addr));
}
...
intptr_t move_bare_placeholder(void *old_addr, SIZE_T new_size,
  ...
  if (!dontunmap)
    windows::release_placeholder(old_addr);
