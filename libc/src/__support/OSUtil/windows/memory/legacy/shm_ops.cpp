...
#include "src/__support/OSUtil/windows/memory/legacy/shm_common.h"
...
intptr_t shm_unlink(const char *name) {
  ...
  auto nt = to_nt_path(path_sv, nt_path, shm_common::SHM_WIDE_BUF);
  if (!nt.has_value()) {
    return -nt.error();
  }
  size_t nt_len = nt.value();

  windows::nt_wstring_view nt_name(nt_path, nt_len);
