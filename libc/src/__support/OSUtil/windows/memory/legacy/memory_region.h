...
LIBC_INLINE bool query_region_info(const void *addr, RegionInfo *info) {
...
LIBC_INLINE void release_range_placeholders(void *addr, SIZE_T size) {
  ...
  for (;;) {
    ...
    // Tear down the NT-side state. nt_pal::free_placeholder works for both
    // unmapped section views and bare placeholder reservations; nt_pal::free_va
    ...
    if (rd != nullptr && rd->section_handle != nullptr) {
      (void)nt_pal::unmap_view_preserve(pick.base);
      (void)nt_pal::free_placeholder(pick.base);
    } else {
      (void)nt_pal::free_va(pick.base);
    }
...
/// via decommit + nt_pal::preserve_to_placeholder (no MEM_FREE window during
...
[[nodiscard]] LIBC_INLINE bool prepare_for_fixed(void *addr, SIZE_T size) {
  ...
  // teardown by shape (anon → nt_pal::free_va, section-backed →
  // nt_pal::unmap_view_preserve + nt_pal::free_placeholder), and skips FOREIGN. The
  ...
  // nt_pal::reserve_placeholder over?"
  //
  ...
  struct GapFill {
    ...
    bool ok;          // false on the first failed nt_pal::reserve_placeholder
  };
  ...
  auto fill_gap = +[](GapFill &g, char *gap_end) {
    ...
    if (is_alloc_aligned(g.cursor)) {
      if (!nt_pal::reserve_placeholder(g.cursor, gap_size))
        g.ok = false;
    } else {
      ...
      if (!nt_pal::reserve_placeholder(g.run_start, merged_size))
        g.ok = false;
    }
    if (!g.run_start)
  ...
  NTSTATUS st = nt_pal::coalesce_placeholders(addr, size);
  if (NT_ERROR(st))
    return false;
...
LIBC_INLINE void destroy_ring_buffer(void *ring, SIZE_T size) {
  ...
  nt_pal::unmap_view(primary);
  nt_pal::unmap_view(alias);
...
// apply_vm_info_per_region → nt_pal::for_committed_batched (nt_pal/query.h)
// prefetch_committed_regions → nt_pal::prefetch_committed (nt_pal/query.h)

// reset_pages → nt_pal::vm_reset (nt_pal/placeholder.h)
// dontneed_private_pages → nt_pal::vm_dontneed (nt_pal/placeholder.h)
