...
void flush_untracked_mapping(HANDLE process, void *addr) {
  ...
  // sizeof(UNICODE_STRING) + MAX_PATH WCHARs covers all practical paths.
  constexpr SIZE_T BUF_SIZE = sizeof(UNICODE_STRING) + 520 * sizeof(WCHAR);
  auto scratch = windows::byte_scratch(BUF_SIZE);
  if (!scratch)
    return;

  SIZE_T return_length = 0;
  NTSTATUS status = ::NtQueryVirtualMemory(
      process, addr, MemoryMappedFilenameInformation,
  ...
  InitializeObjectAttributes(&oa, name, OBJ_CASE_INSENSITIVE, nullptr,
                             nullptr);
  IO_STATUS_BLOCK io_status;
  windows::ScopedNtHandle fh;
  status = ::NtOpenFile(fh.put(), FILE_WRITE_DATA | SYNCHRONIZE, &oa,
                        &io_status,
...
void revert_cow_pages(HANDLE process, char *start, char *end) {
  // Bulk-capture the MBI walk via nt_pal::RegionWalker. PAGE_REVERT_TO_FILE_MAP
  // does not change VA layout, so an upfront snapshot is safe.
  auto ws = windows::byte_scratch(4096);
  if (!ws)
    return; // best-effort; matches the existing failure tolerance
  nt_pal::RegionWalker walk(start, static_cast<SIZE_T>(end - start),
                             ws.data(), ws.size());
  while (walk.next()) {
    ...
    if (!windows::g_mapping_table.snapshot(walk.entry->AllocationBase, &snap))
      continue;
    if (snap.region == nullptr || snap.region->file_handle == nullptr)
      continue;
    // This is a COW'd page within a file view. Revert it.
    PVOID base = walk.chunk;
    SIZE_T chunk_size = walk.chunk_size;
    ULONG old_prot;
...
intptr_t msync(void *addr, size_t len, int flags) {
  ...
  auto ws = windows::byte_scratch(4096);
  if (!ws) return -ENOMEM;
  nt_pal::RegionWalker walk(cur, static_cast<SIZE_T>(range_end - cur),
    ...
    if (has_sync && walk.entry->State == MEM_COMMIT &&
      ...
      if (walk.entry->Type == MEM_MAPPED) {
        ...
        if (windows::g_mapping_table.snapshot(walk.entry->AllocationBase,
                                              &flush_snap) &&
            flush_snap.region != nullptr &&
            flush_snap.region->file_handle != nullptr) {
          flush_file_handle(flush_snap.region->file_handle);
        } else {
