...
bool RemapGuard::unmap() {
  ...
  NTSTATUS st = nt_pal::unmap_view_preserve_transient(vb);
  if (NT_ERROR(st))
...
void RemapGuard::rollback_from_unmapped() {
  ...
  //
  // nt_pal::RegionWalker drives a paginated NtPssCaptureVaSpaceBulk over the bound,
  ...
  bool walk_ok = false;
  {
    ...
    if (ws) {
      ...
      nt_pal::RegionWalker walk(bound, guarded_size_, ws.data(), ws.size());
      while (walk.next()) {
        if (walk.entry->Type == MEM_MAPPED) {
          nt_pal::unmap_view_preserve(walk.entry->BaseAddress);
        } else if (walk.entry->State == MEM_COMMIT &&
                   walk.entry->Type == MEM_PRIVATE) {
          nt_pal::preserve_to_placeholder(walk.chunk, walk.chunk_size);
        }
      }
      walk_ok = true;
    }
