...
intptr_t madvise(void *addr, size_t size, int advice) {
  ...
  case MADV_DONTNEED: {
    ...
    while (walk.next()) {
      ...
      if (walk.entry->Type == MEM_PRIVATE) {
        if (!nt_pal::vm_dontneed(walk.chunk, walk.chunk_size,
                                  walk.entry->AllocationProtect))
          dontneed_failed = true;
        ...
        if (is_writecopy && file_backed) {
          ...
          nt_pal::decommit_private_range(walk.chunk, walk.chunk_size);
        } else {
          windows::wsex_dontneed_section_pages(walk.chunk, walk.chunk_size);
        }
      }
    }
    // A failed private-mapping decommit+recommit leaves the chunk
    // decommitted; Linux surfaces this as ENOMEM. Other per-chunk paths
    // are advisory and stay best-effort.
    return dontneed_failed ? -ENOMEM : 0;
  }
  ...
  case MADV_FREE: {
    ...
    while (walk.next()) {
      ...
      if (walk.entry->Type == MEM_MAPPED) {
        ...
        nt_pal::vm_reset(walk.chunk, walk.chunk_size);
      }
    }
    return 0;
  }
  ...
  //==========================================================================
  case MADV_WILLNEED: {
    // nt_pal::vm_reset_undo and nt_pal::commit_in_reservation_no_writewatch mutate the current chunk only -- adjacent
    ...
    while (walk.next()) {
      ...
      if (walk.entry->State == MEM_RESERVE && walk.entry->Type == MEM_MAPPED) {
        ...
        if (windows::g_mapping_table.snapshot(
            ...
            snap.region != nullptr &&
            snap.region->has_flag(windows::memory::region_flag::NORESERVE)) {
          nt_pal::commit_in_reservation_no_writewatch(walk.chunk, walk.chunk_size, snap.view_prot);
        }
      }
    }
