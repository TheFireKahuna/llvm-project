...
//     (\Sessions\<N>\BaseNamedObjects\llvm_libc_sysv_shm_reg). Contains an
//     array of ShmSlot entries with atomic state management for lock-free
//     access. Session-scoped so normal user processes can create it
//     without SeCreateGlobalPrivilege.
//   - Each data segment is a separate named section
//     (\Sessions\<N>\BaseNamedObjects\llvm_libc_sysv_shm_<id>).
...
// Valid entries (shm views inherited via RtlCloneUserProcess address
...
static void sysv_shm_fork_reinit_impl() {
  ...
  // POSIX / SysV: each shared-memory attachment inherited by the child must
  // increment shm_nattch in the shared registry. Without this bump the
  // parent's shmdt() drops nattch to 0 and — if the segment is IPC_RMID'd —
  // the backing section is released while the child still has a live view.
  //
  // The registry section is process-shared (named section), so the child's
  // view of `reg->slots` is the same memory as the parent's. We access it
  // via get_registry(); the lazy-init path re-opens the mapping in the
  // child if the parent never touched sysv_shm post-registry-init (in that
  // case count==0 and the loop below does nothing).
  if (dst == 0)
    return;

  ShmRegistry *reg = get_registry();
  if (!reg)
    return; // Registry unavailable — caller will observe stale nattch;
            // no safe recovery from here.

  for (uint32_t i = 0; i < dst; ++i) {
    int shmid = g_attaches[i].shmid;
    for (size_t s = 0; s < MAX_SHM_SEGMENTS; ++s) {
      uint32_t st = reg->slots[s].state.load(cpp::MemoryOrder::ACQUIRE);
      if ((st == SHM_SLOT_LIVE || st == SHM_SLOT_REMOVED) &&
          reg->slots[s].shmid == shmid) {
        reg->slots[s].shm_nattch.fetch_add(1, cpp::MemoryOrder::RELAXED);
        break;
      }
    }
  }
...
static ShmRegistry *get_registry() {
  ...
  // Build the section name: \Sessions\<N>\BaseNamedObjects\llvm_libc_sysv_shm_reg
  ...
  g_registry_region = windows::SectionRegion::create_named(
      &reg_wsv, sizeof(ShmRegistry), PAGE_READWRITE, nullptr, &st);
  if (!g_registry_region)
    return nullptr;
...
// Build \Sessions\<N>\BaseNamedObjects\llvm_libc_sysv_shm_<id>
...
static size_t build_segment_name(int shmid, WCHAR *buf, size_t max_wchars) {
  ...
  windows::write_session_bno_prefix(ss);
  ss << u"llvm_libc_sysv_shm_" << static_cast<unsigned int>(shmid);
  if (ss.overflow())
    return 0;
  ss.null_terminate();
  return ss.str().size();
...
intptr_t shmget(key_t key, size_t size, int shmflg) {
  ...
  size_t name_len = build_segment_name(id, name_s.data(), name_s.size());
  if (name_len == 0) {
    slot->state.store(SHM_SLOT_EMPTY, cpp::MemoryOrder::RELEASE);
    return -EINVAL;
  }

  windows::nt_wstring_view seg_wsv(name_s.data(), name_len);
  ...
  // Populate slot metadata. Per POSIX, at creation time uid == cuid and
  // gid == cgid (the effective identity of the creating process). The
  // cuid/cgid fields are immutable; only uid/gid may be changed via
  // IPC_SET, gated on caller == creator/owner/privileged.
  ...
  slot->shm_perm_uid = uid;
  slot->shm_perm_gid = gid;
  slot->shm_perm_cuid = uid;
  slot->shm_perm_cgid = gid;
...
intptr_t shmat(int shmid, const void *shmaddr, int shmflg) {
  ...
  if (!name_s)
    return -ENOMEM;
  size_t name_len = build_segment_name(shmid, name_s.data(), name_s.size());
  if (name_len == 0)
    return -EINVAL;
  ...
  windows::SectionHandle section =
      windows::SectionHandle::open_named(&seg_wsv, access, &nt_st);
  if (!section)
    return -EINVAL;
...
LIBC_REGISTER_FORK_REINIT(sysv_shm,
                          ::LIBC_NAMESPACE::internal::kForkPrioSysvShm,
                          &::LIBC_NAMESPACE::internal::sysv_shm_fork_reinit)
