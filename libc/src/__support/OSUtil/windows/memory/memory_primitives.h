...
//   Release     — vm_release
//   One-shot    — oneshot_alloc, oneshot_alloc_watched, oneshot_alloc_32bit,
//                 oneshot_alloc_numa, interior_release, section_hint_map
...
//===----------------------------------------------------------------------===//
// One-shot — Atomic reserve+commit for anonymous private mappings
//===----------------------------------------------------------------------===//
//
// One-shot allocations bypass the placeholder model entirely.
// NtAllocateVirtualMemoryEx(MEM_RESERVE|MEM_COMMIT) atomically reserves
// and commits in a single kernel call — the VAD lock serializes against
// all other VA operations, so there is no window for VA races.
//
// One-shot allocations appear as MEM_PRIVATE in MBI queries. They support:
//   - Interior release (MEM_RELEASE on sub-ranges) for partial munmap
//   - Recommit (MEM_COMMIT on decommitted sub-ranges) for MAP_FIXED anon
//   - Standard decommit/protect/reset operations
//
// They do NOT support MEM_PRESERVE_PLACEHOLDER or MEM_REPLACE_PLACEHOLDER.
// When MAP_FIXED targets one-shot VA with a file/shared mapping, the
// promotion path is: interior_release() → section_hint_map().
//
// The MBI Type field (MEM_PRIVATE vs MEM_MAPPED) is the dispatch key.
// No mapping table entry, no userspace metadata, no flags.

/// Atomic reserve+commit. Returns the base address on success, nullptr
/// on failure. The kernel chooses the address when hint is nullptr.
/// When hint is non-null, the kernel attempts that address and falls
/// back to a kernel-chosen address if the hint is occupied.
///
/// Supports extended parameters (NUMA node, address requirements) via
/// the params/param_count arguments. Pass nullptr/0 for defaults.
///
/// This is the hot-path primitive for mmap(NULL, MAP_PRIVATE|MAP_ANONYMOUS).
/// One syscall. 40% faster than the placeholder two-step at 64-256KB.
LIBC_INLINE void *oneshot_alloc(void *hint, SIZE_T size, DWORD page_prot,
                                MEM_EXTENDED_PARAMETER *params = nullptr,
                                ULONG param_count = 0) {
  PVOID base = hint;
  SIZE_T actual = size;
  NTSTATUS st = ::NtAllocateVirtualMemoryEx(
      NtCurrentProcess(), &base, &actual, MEM_RESERVE | MEM_COMMIT, page_prot,
      params, param_count);
  return NT_SUCCESS(st) ? base : nullptr;
}

/// One-shot alloc with hardware dirty page tracking (MEM_WRITE_WATCH).
/// Enables per-page write-watch bitmap at zero additional cost (the flag is
/// added to the same NtAllocateVirtualMemoryEx call). The bitmap is queried
/// via write_watch_query/write_watch_query_reset and tracks only genuine
/// user writes — demand-read via registered IoRing buffers does NOT trigger
/// dirty (MDL bypass), giving perfect CoW semantics for fork optimization.
///
/// Incompatible with MEM_LARGE_PAGES and MEM_RESERVE_PLACEHOLDER.
LIBC_INLINE void *oneshot_alloc_watched(void *hint, SIZE_T size,
                                        DWORD page_prot,
                                        MEM_EXTENDED_PARAMETER *params = nullptr,
                                        ULONG param_count = 0) {
  PVOID base = hint;
  ...
  NTSTATUS st = ::NtAllocateVirtualMemoryEx(
      NtCurrentProcess(), &base, &actual,
      MEM_RESERVE | MEM_COMMIT | MEM_WRITE_WATCH, page_prot, params,
      param_count);
  return NT_SUCCESS(st) ? base : nullptr;
}

/// One-shot alloc constrained to the low 2 GB (MAP_32BIT equivalent).
LIBC_INLINE void *oneshot_alloc_32bit(SIZE_T size, DWORD page_prot) {
  MEM_ADDRESS_REQUIREMENTS reqs = {};
  reqs.HighestEndingAddress =
      reinterpret_cast<PVOID>(static_cast<ULONG_PTR>(0x7FFFFFFFU));
  ...
  return oneshot_alloc(nullptr, size, page_prot, &param, 1);
}

/// One-shot alloc with write-watch, constrained to the low 2 GB.
LIBC_INLINE void *oneshot_alloc_32bit_watched(SIZE_T size, DWORD page_prot) {
  MEM_ADDRESS_REQUIREMENTS reqs = {};
  reqs.HighestEndingAddress =
      reinterpret_cast<PVOID>(static_cast<ULONG_PTR>(0x7FFFFFFFU));
  ...
  return oneshot_alloc_watched(nullptr, size, page_prot, &param, 1);
}

/// One-shot alloc with NUMA node affinity.
LIBC_INLINE void *oneshot_alloc_numa(void *hint, SIZE_T size, DWORD page_prot,
                                     ULONG numa_node) {
  MEM_EXTENDED_PARAMETER param = {};
  param.Type = MemExtendedParameterNumaNode;
  param.ULong = numa_node;

  return oneshot_alloc(hint, size, page_prot, &param, 1);
}
...
///
/// This is the partial-munmap primitive for one-shot allocations. It
/// replaces the placeholder decommit+preserve+coalesce sequence with a
/// single syscall that directly returns VA to the system.
///
/// Also serves as the first step of the promotion path: punch a hole
/// in one-shot VA, then fill it with section_hint_map().
///
/// Returns true on success, false on failure (e.g., addr is not within
/// a MEM_PRIVATE allocation, or the range spans multiple allocations).
...
// Note: recommit of decommitted sub-ranges within one-shot allocations
// uses vm_commit() directly — it already handles MEM_COMMIT on any
// MEM_RESERVE sub-range within a MEM_PRIVATE allocation. No separate
// primitive needed. See the Commit section above.

/// Map a section view at a specific MEM_FREE address using the kernel's
/// address hint mechanism. No placeholder involvement — the kernel maps
/// the section directly into free VA at the requested address.
///
/// This is the second step of the promotion path for MAP_FIXED
/// file/shared into one-shot VA:
///   1. interior_release(target, size) — punch to MEM_FREE
///   2. section_hint_map(section, target, size, offset, prot) — fill
///
/// The hint is honored with ~100% reliability under mmap_lock
/// serialization (0% failure without contention, ~2.5% under extreme
/// synthetic VA churn from concurrent DLL loads). On failure, returns
/// a non-success NTSTATUS — the caller falls back to the freeze
/// bracket (NtCreateThreadStateChange) or placeholder promotion.
///
/// 31% faster than the direct placeholder path (1515 ns vs 2199 ns)
/// because it skips placeholder reservation and MEM_REPLACE_PLACEHOLDER
/// validation overhead.
///
/// Supports extended parameters (NUMA affinity) via params/param_count.
LIBC_INLINE NTSTATUS section_hint_map(HANDLE section, void *target,
                                      SIZE_T size, LARGE_INTEGER offset,
                                      DWORD page_prot,
                                      MEM_EXTENDED_PARAMETER *params = nullptr,
                                      ULONG param_count = 0) {
  PVOID base = target;
  SIZE_T sz = size;
  return ::NtMapViewOfSectionEx(section, NtCurrentProcess(), &base, &offset,
                                &sz, 0, page_prot, params, param_count);
}
...
/// Coalesce a freed placeholder with adjacent placeholders, then release
/// if isolated (MEM_FREE on both sides). Prevents sub-64KB unrepairable
/// VA gaps while avoiding micro-accumulation of orphaned placeholders.
///
/// \p addr / \p size describe the freed placeholder range.
///
/// Invariant after this call:
///   - Freed placeholder exists iff adjacent to a live mapping.
///   - Isolated freed placeholders are released to MEM_FREE.
///   - Adjacent freed placeholders are coalesced before release check.
///
/// Uses MemoryRegionInformationEx (class 7) for definitive placeholder
/// detection via the PlaceholderReservation bit — no false positives
/// from regular MEM_RESERVE regions. MRI also returns the full allocation
/// extent in RegionSize, giving the neighbor's exact boundaries in one
/// query. Cross-allocation coalesce is supported by the kernel.
///
/// Cost: 2 MRI queries (neighbors) + 0-2 coalesce + 0-1 release = 2-5 syscalls.
LIBC_INLINE void coalesce_or_release_placeholder(void *addr, SIZE_T size) {
  HANDLE process = NtCurrentProcess();
  char *ph_start = static_cast<char *>(addr);
  char *ph_end = ph_start + size;

  // Query left neighbor via MRI for definitive placeholder detection.
  MEMORY_REGION_INFORMATION left_mri = {};
  bool has_left_ph = false;
  if (ph_start > reinterpret_cast<char *>(get_min_address())) {
    if (query_region_mri(ph_start - 1, left_mri) &&
        left_mri.PlaceholderReservation) {
      has_left_ph = true;
    }
  }

  // Query right neighbor.
  MEMORY_REGION_INFORMATION right_mri = {};
  bool has_right_ph = false;
  if (query_region_mri(ph_end, right_mri) &&
      right_mri.PlaceholderReservation) {
    has_right_ph = true;
  }

  // Coalesce with neighbors. MRI.RegionSize gives the full allocation
  // extent, so AllocationBase + RegionSize is the exact neighbor end.
  char *coal_start = ph_start;
  char *coal_end = ph_end;

  if (has_left_ph) {
    char *left_base = static_cast<char *>(left_mri.AllocationBase);
    SIZE_T span = static_cast<SIZE_T>(coal_end - left_base);
    if (NT_SUCCESS(vm_coalesce_placeholders(left_base, span)))
      coal_start = left_base;
  }

  if (has_right_ph) {
    char *right_end = static_cast<char *>(right_mri.AllocationBase) +
                      right_mri.RegionSize;
    SIZE_T span = static_cast<SIZE_T>(right_end - coal_start);
    if (NT_SUCCESS(vm_coalesce_placeholders(coal_start, span)))
      coal_end = right_end;
  }

  // Check if the (possibly coalesced) placeholder is isolated.
  // MRI fails on MEM_FREE addresses, so failure = free.
  bool left_free = false;
  bool right_free = false;

  if (coal_start <= reinterpret_cast<char *>(get_min_address())) {
    left_free = true; // At bottom of address space.
  } else if (!has_left_ph) {
    // Already queried left — not a placeholder. Free iff query failed.
    left_free = (left_mri.AllocationBase == nullptr);
  } else {
    // Left was coalesced — query the new boundary.
    MEMORY_REGION_INFORMATION mri;
    left_free = !query_region_mri(coal_start - 1, mri);
  }

  if (!has_right_ph) {
    // Already queried right — not a placeholder. Free iff query failed.
    right_free = (right_mri.AllocationBase == nullptr);
  } else {
    // Right was coalesced — query the new boundary.
    MEMORY_REGION_INFORMATION mri;
    right_free = !query_region_mri(coal_end, mri);
  }

  if (left_free && right_free) {
    // Isolated — safe to release. No unrepairable gap.
    PVOID base = coal_start;
    SIZE_T sz = 0;
    ::NtFreeVirtualMemory(process, &base, &sz, MEM_RELEASE);
  }
  // Otherwise: sandwiched by live mapping(s). Keep as placeholder.
  // It will coalesce and release when the neighbor is eventually unmapped.
}
