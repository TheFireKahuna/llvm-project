...
LIBC_INLINE size_t device_path_to_dos(const WCHAR *nt_path,
  ...
  UNICODE_STRING mm_name;
  WCHAR mm_path[] = u"\\Device\\MountPointManager";
  mm_name.Buffer = mm_path;
  mm_name.Length = sizeof(mm_path) - sizeof(WCHAR);
  mm_name.MaximumLength = sizeof(mm_path);

  OBJECT_ATTRIBUTES oa;
  oa.Length = sizeof(oa);
  oa.RootDirectory = nullptr;
  oa.ObjectName = &mm_name;
  oa.Attributes = 0;
  ...
  HANDLE mm_handle;
  NTSTATUS status =
      ::NtOpenFile(&mm_handle, SYNCHRONIZE | FILE_READ_ATTRIBUTES, &oa, &iosb,
  ...
  for (ULONG i = 0; i < device_prefix_len; ++i)
    target->DeviceName[i] = nt_path[i];
  ...
  status = ::NtDeviceIoControlFile(
      mm_handle, nullptr, nullptr, nullptr, &iosb,
  ...
  for (ULONG i = 0; i < mount_chars; ++i)
    dos_buf[i] = paths->MultiSz[i];
  for (ULONG i = 0; i < tail_chars; ++i)
    dos_buf[mount_chars + i] = tail[i];
  dos_buf[total] = u'\0';
