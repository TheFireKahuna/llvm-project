...
intptr_t openat(int dirfd, const char *path, int flags, mode_t mode) {
  ...
  ResolvedPath rp = path_detail::resolve_path(sv, path_buf, path_buf_s.size());
  if (rp.error)
  ...
  if (rp.kind == PathKind::DosRelative && !sv.empty()) {
  ...
  if (is_device_node) {
    ...
    if (flags & (O_CREAT | O_EXCL | O_TRUNC))
      return -EINVAL;
    name = windows::nt_wstring_view(path_buf, rp.nt_len);
    ...
    int err = resolve_at_path(dirfd, sv, path_buf, path_buf_s.size(),
                              &oa, &name);
    if (err)
...
intptr_t dup(int oldfd) {
  // Linux dup(fd): returns the lowest-available fd (>=0) sharing the OFD.
  // fd_table.dup already implements the lowest-free search with min_fd=0.
  auto result = fd_table.dup(oldfd, 0, 0);
  if (!result.has_value())
    return -static_cast<intptr_t>(result.error());
  return static_cast<intptr_t>(result.value());
}
