...
struct ThreadRing {
  ...
  HANDLE event;           // IoRing completion event — reactor WCP consumer.
                          // Also used for bounded EINTR cancel drain
                          // (NtWaitForSingleObject with short timeout).
  ...
  // thread calls ThreadLocalWord::alert_if_parked(&cq_word) — read-only
  // park_state_ check + conditional NtAlert. One-shot, re-armed in callback.
  ...
  // ---------------------------------------------------------------------------
  // CQ notification word — ThreadLocalWord
  // ---------------------------------------------------------------------------
  //
  // Single-owner primitive replacing the old needs_wake tri-state + parking
  // lot + wait_event dual path. Zero-bounce design: the reactor waker only
  // READS park_state_ (via alert_if_parked) — no writes to value_ or any
  // field on the owner's cache line.
  //
  // Memory layout: cq_word lives on its own cache line (alignas(64)), separate
  // from the ring's hot SQE/CQE fields. The reactor only reads cq_word
  // (park_state_ snoop to Shared); the owner writes ring.sq_tail/cq_head
  // (same-thread). No false sharing, no write-invalidation.
  //
  // Wake path (reactor → owner):
  //   alert_if_parked(&cq_word) →
  //     load park_state_ (ACQUIRE, read-share ~20-40ns)
  //     if KERNEL_PARKED: NtAlertThreadByThreadId(owner_tid)
  //
  // Sleep path (owner):
  //   wait_for_addr<Interruptible>(&cq->Tail, old_tail) →
  //     Phase 0: UMWAIT/MWAITX on cq->Tail (kernel write, TSC-bounded)
  //     Phase 1: NtWaitForAlertByThreadId (Dekker on cq->Tail + park_state_)
  //
  alignas(64) ThreadLocalWord cq_word;
};
...
//
// No `.libcfin` registration for the ring layer itself: every ThreadRing
// is owned by a ThreadLifecycle and freed when its owner thread exits via
// lifecycle_cleanup → thread_ring_destroy. The backing storage comes
// from the lifecycle's own SlabPool (torn down at $P4 with the rest of
// the pools), so there is no separate ring_pool that could outlive its
// allocator. The ring carries no process-wide kernel handle that would
// require a coordinated drop at fini — IORING handles, completion
// events, and reactor tokens are all owned per-instance and released
// inside thread_ring_destroy.
