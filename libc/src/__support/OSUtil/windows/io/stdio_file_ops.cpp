...
intptr_t freopen_impl(const char *path, const char *mode, ::FILE *stream,
  ...
  if (path) {
    path_len = to_nt_path(path, path_buf, path_s.size());
    if (path_len == 0)
      return -EINVAL;
  }
...
intptr_t tmpfile_impl(::FILE **out) {
  ...
  static constexpr WCHAR PREFIX[] = u"libc_";
  static constexpr size_t PREFIX_LEN = 5;

  // +4 for \??\ prefix, +1 for separator, +1 for NUL terminator.
  if (4 + dir_len + 1 + PREFIX_LEN + NAME_HEX_CHARS + 1 > PATH_SCRATCH_WCHARS)
    return -ENAMETOOLONG;
  ...
  for (size_t i = 0; i < dir_len; ++i)
    path_buf[pos++] = temp_dir[i];

  // Re-add path separator between directory and filename.
  path_buf[pos++] = u'\\';
  ...
  for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
    ...
    UNICODE_STRING us;
    OBJECT_ATTRIBUTES oa;
    init_object_attributes(&oa, &us, path_buf, pos);
...
intptr_t popen_impl(const char *command, const char *mode, ::FILE **out) {
  ...
  if (ret == -ENOENT || ret == -EACCES) {
    ...
    size_t shell_len = __builtin_strlen(shell);
    size_t cmd_len = __builtin_strlen(command);
    // Layout: '"' shell '"' ' /c ' command '\0'
    size_t raw_len = 1 + shell_len + 1 + 4 + cmd_len + 1;
    void *raw_buf =
    ...
    char *p = raw_cmdline;
    *p++ = '"';
    __builtin_memcpy(p, shell, shell_len);
    p += shell_len;
    *p++ = '"';
    *p++ = ' ';
    *p++ = '/';
    *p++ = 'c';
    *p++ = ' ';
    __builtin_memcpy(p, command, cmd_len);
    p += cmd_len;
    *p = '\0';
