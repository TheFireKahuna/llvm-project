...
intptr_t fsync(int fd) {
  ...
  FileIOResult result = ioring_submit_and_wait(ring, tr->event, tr->wait_event,
                                               h, gen);
  if (result.has_error())
    return -result.error;
  return 0;
...
intptr_t truncate(const char *path, off_t length) {
  ...
  NTSTATUS status = ::NtOpenFile(
      &handle, FILE_WRITE_DATA | SYNCHRONIZE, &oa, &iosb,
  ...
  status = ::NtSetInformationFile(handle, &iosb, &eof_info,
...
intptr_t pipe2(int pipefd[2], int flags) {
  ...
  // Empty UNICODE_STRING = anonymous pipe.
  UNICODE_STRING empty_name;
  empty_name.Length = 0;
  empty_name.MaximumLength = 0;
  empty_name.Buffer = nullptr;
...
intptr_t copy_file_range(int fd_in, off_t *off_in, int fd_out, off_t *off_out,
  ...
  NTSTATUS es = ::NtCreateEvent(&evt, EVENT_MODIFY_STATE | SYNCHRONIZE, &oa,
  ...
  while (total < len) {
    ...
    IO_STATUS_BLOCK iosb = {};
    NTSTATUS s = ::NtCopyFileChunk(h_in, h_out, evt, &iosb, chunk,
                                   &src_offset, &dst_offset,
                                   nullptr, nullptr, 0);
    if (s == STATUS_PENDING) {
      s = alertable_wait_restartable(evt);
