...
static ssize_t ioring_simple_read(HANDLE h, void *buf, size_t count) {
  ...
  FileIOResult result = read_single_op_wait(tr, h, buf,
                                            io_len, 0, gen);
  if (result.has_error())
    return -result.error;
...
static ssize_t ioring_simple_write(HANDLE h, const void *buf, size_t count) {
  ...
  ULONG io_len = (count > 0xFFFFFFFFULL) ? 0xFFFFFFFFU
                                         : static_cast<ULONG>(count);
  uint32_t gen = tr->next_generation();
  FileIOResult result = write_single_op_wait(tr, h, buf,
                                             io_len, 0, gen);
  if (result.has_error())
    return -result.error;
  return static_cast<ssize_t>(result.value);
...
ssize_t disk_write_impl(OpenFileDescription *ofd, const void *buf,
  ...
  uint32_t gen = tr->next_generation();
  FileIOResult result = write_single_op_wait(tr, h, buf, io_len, offset, gen);
...
ssize_t pipe_read_impl(OpenFileDescription *ofd, void *buf, size_t count) {
  ...
  FileIOResult result = ioring_pipe_io(tr, h, gen, pflags & O_NONBLOCK);
  if (result.has_error()) {
    if (result.error == EPIPE)
      return 0; // EOF
    return -result.error;
...
bool afd_release_aux(OpenFileDescription *ofd) {
  auto *state = ofd->afd_socket();
  // Drain any pending nonblocking-connect IRP. The kernel's eventual
  // completion posts to the reactor IOCP with ApcContext pointing into
  // SocketState; freeing the state ahead of that post would UAF.
  // release() closes ofd->handle after this returns, so cancelling now
  // is the last opportunity to intercept the IRP on its own handle.
  socket_drain_pending_connect(ofd->handle, state);
  socket_state_free(state);
  return true;
}
...
// ---------------------------------------------------------------------------
// FileOps instances for kinds whose read/write/release_aux all live in this
// TU (afd_socket, socketpair, fifo, pty_master, pty_slave). read_write.cpp
// is always pulled once any I/O is linked, so these registrations always
// appear in .libcops$*.
// ---------------------------------------------------------------------------
...
// Register all five into .libcops$M. Since read_write.cpp is
// always-pulled, these records are always present in c.dll; a
// hermetic bench that doesn't pull read_write.cpp (should never
// happen given always-pulled kinds) would simply see them absent.
#include "src/__support/OSUtil/windows/fd/file_ops_section.h"
LIBC_REGISTER_FILE_OPS(AfdSocket, afd_socket_ops)
LIBC_REGISTER_FILE_OPS(SocketPair, socketpair_ops)
LIBC_REGISTER_FILE_OPS(Fifo, fifo_ops)
LIBC_REGISTER_FILE_OPS(PtyMaster, pty_master_ops)
LIBC_REGISTER_FILE_OPS(PtySlave, pty_slave_ops)
