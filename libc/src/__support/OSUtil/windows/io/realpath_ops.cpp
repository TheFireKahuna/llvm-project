...
intptr_t realpath(const char *__restrict path, char *__restrict resolved_path) {
  ...
  size_t nt_len = to_nt_path(path, nt_path_buf, nt_s.size());
  if (nt_len == 0)
    return -EINVAL;

  UNICODE_STRING us;
  OBJECT_ATTRIBUTES oa;
  init_object_attributes(&oa, &us, nt_path_buf, nt_len);

  IO_STATUS_BLOCK iosb = {};
  HANDLE file_handle;
  NTSTATUS st = ::NtOpenFile(
      &file_handle, FILE_READ_ATTRIBUTES | SYNCHRONIZE, &oa, &iosb,
  ...
  st = ::NtQueryObject(file_handle, ObjectNameInformation, name_s.data(),
  ...
  ULONG utf8_bytes = 0;
  st = ::RtlUnicodeToUTF8N(nullptr, 0, &utf8_bytes, dos_buf,
                            static_cast<ULONG>(dos_len * sizeof(WCHAR)));
  if (!NT_SUCCESS(st) || utf8_bytes == 0)
    return -EILSEQ;
  ...
  ULONG written = 0;
  st = ::RtlUnicodeToUTF8N(out, utf8_bytes, &written, dos_buf,
                            static_cast<ULONG>(dos_len * sizeof(WCHAR)));
  if (!NT_SUCCESS(st)) {
