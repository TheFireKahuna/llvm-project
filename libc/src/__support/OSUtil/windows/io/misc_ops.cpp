...
int query_hostname() {
  ...
  ULONG utf8_len = 0;
  status = ::RtlUnicodeToUTF8N(cached_hostname, MAX_HOSTNAME - 1, &utf8_len,
                                wide_data, wide_bytes);
  if (!NT_SUCCESS(status))
    return -1;

  cached_hostname[utf8_len] = '\0';
  return static_cast<int>(utf8_len);
}
...
intptr_t gethostname(char *name, size_t len) {
  ...
  // Copy cached hostname to caller's buffer.
  for (int i = 0; i <= hostname_len; ++i)
    name[i] = cached_hostname[i];
...
ErrorOr<long> pathconf_handle_internal(HANDLE file_handle, int name) {
  ...
  case _PC_FILESIZEBITS:
    ...
    if (attr) {
      // FAT32's FileSystemName is L"FAT32" (10 bytes).
      ULONG fs_len = attr->FileSystemNameLength / sizeof(WCHAR);
      if (fs_len >= 3 && attr->FileSystemName[0] == L'F' &&
          attr->FileSystemName[1] == L'A' &&
          attr->FileSystemName[2] == L'T')
        return 32L;
    }
    return 64L;
  ...
  case _PC_TIMESTAMP_RESOLUTION:
    ...
    if (attr) {
      ULONG fs_len = attr->FileSystemNameLength / sizeof(WCHAR);
      if (fs_len >= 5 && attr->FileSystemName[0] == L'e' &&
          attr->FileSystemName[1] == L'x' &&
          attr->FileSystemName[2] == L'F' &&
          attr->FileSystemName[3] == L'A' &&
          attr->FileSystemName[4] == L'T')
...
ErrorOr<long> pathconf(const char *path, int name) {
  ...
  size_t nt_len = to_nt_path(path, path_buf, path_buf_s.size());
  if (nt_len == 0)
    return Error(EINVAL);

  UNICODE_STRING us;
  OBJECT_ATTRIBUTES oa;
  init_object_attributes(&oa, &us, path_buf, nt_len);
