...
LIBC_INLINE DWORD tls_alloc() {
  ...
  while (old != ~uint64_t{0}) {
    ...
    if (bits->compare_exchange_weak(old, want,
      ...
      return idx;
    // CAS failed — 'old' is updated, retry. Adaptive cascade: 4 bare
    // PAUSE retries absorb weak-CAS spurious failure and 2-way races,
    // then UMWAIT/MWAITX caps cycle cost under parallel TLS-init bursts.
    if (LIBC_LIKELY(attempts++ < 4)) {
      spin_wait::relax_processor();
    } else {
      spin_wait::spin_on_raw(
          reinterpret_cast<uint64_t *>(&bits->val), old);
      old = bits->load(cpp::MemoryOrder::RELAXED);
      attempts = 0;
    }
  }
...
LIBC_INLINE void tls_free(DWORD index) {
  ...
  // Single LOCK AND — atomically clear the target bit. No CAS retry
  // needed: unlike tls_alloc (whose new bit depends on the current
  // pattern), tls_free's result is a fixed-mask AND.
  uint64_t mask = ~(uint64_t{1} << index);
  bits->fetch_and(mask, cpp::MemoryOrder::ACQ_REL);
}
