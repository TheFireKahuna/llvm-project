...
// thread-exit cleanup via .CRT$XL or equivalent. Derived via `decltype`
// on an unreferenced external prototype (LIBC_MSABI can only decorate
// a function *declaration*, not a type alias).
LIBC_MSABI void __fls_callback_type_source(PVOID);
using FLS_CALLBACK = decltype(&__fls_callback_type_source);
