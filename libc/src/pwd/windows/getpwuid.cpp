//===-- Windows implementation of getpwuid --------------------------------===//
//
...
// POSIX getpwuid: returns a pointer to a static struct passwd, or nullptr.
...
LLVM_LIBC_FUNCTION(struct passwd *, getpwuid, (uid_t uid)) {
  ...
  int err = internal::fill_passwd_uid(uid, &pwd, buf, sizeof(buf));
  if (err == ENOENT)
    return nullptr;
  if (err != 0) {
    libc_errno = err;
