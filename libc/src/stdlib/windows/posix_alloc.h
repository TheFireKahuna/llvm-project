...
//   Per-thread free lists for every slab size class (see
//   posix_alloc_size_classes.h), accessed via FLS/TEB TLS.
...
/// Returns 0 on success, non-zero if a hard prerequisite (TLS slot
/// allocation) failed. The lazy `posix_alloc()` re-entry path also
/// invokes this and asserts success internally.
