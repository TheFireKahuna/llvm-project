//===-- Windows implementation header for select -----------------*- C++ -*-===//
//
// Part of the LLVM Project, under the Apache License v2.0 with LLVM Exceptions.
// See https://llvm.org/LICENSE.txt for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
//===----------------------------------------------------------------------===//
//
// The shared select.h includes <sys/select.h> which doesn't exist on
// Windows without a full runtimes build. This header provides the
// declaration directly using our own type definitions.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIBC_SRC_SYS_SELECT_WINDOWS_SELECT_H
#define LLVM_LIBC_SRC_SYS_SELECT_WINDOWS_SELECT_H

#include "include/llvm-libc-macros/sys-select-macros.h"
#include "include/llvm-libc-types/fd_set.h"
#include "include/llvm-libc-types/struct_timeval.h"
#include "src/__support/macros/config.h"

namespace LIBC_NAMESPACE_DECL {

int select(int nfds, fd_set *__restrict read_set, fd_set *__restrict write_set,
           fd_set *__restrict error_set, struct timeval *__restrict timeout);

} // namespace LIBC_NAMESPACE_DECL

#endif // LLVM_LIBC_SRC_SYS_SELECT_WINDOWS_SELECT_H
