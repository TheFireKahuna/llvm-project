...
typedef EXCEPTION_DISPOSITION(NTAPI EXCEPTION_ROUTINE)(EXCEPTION_RECORD *,
                                                        PVOID, CONTEXT *, PVOID);
...
__declspec(dllimport) PRUNTIME_FUNCTION NTAPI
RtlLookupFunctionEntry(DWORD64 ControlPc, DWORD64 *ImageBase,
                        PVOID HistoryTable);

__declspec(dllimport) PEXCEPTION_ROUTINE NTAPI
...
__declspec(dllimport) void NTAPI RtlCaptureContext(CONTEXT *ContextRecord);
