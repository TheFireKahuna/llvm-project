...
set(CXX_SUPPORTS_UNWINDLIB_EQ_NONE_FLAG ON CACHE INTERNAL
  "Forced ON for NTPOSIX — link check cannot succeed during bootstrap")
set(CXX_SUPPORTS_NOLIBC_FLAG ON CACHE INTERNAL
  "Forced ON for NTPOSIX — link check cannot succeed during bootstrap")
...
add_link_options($<$<STREQUAL:$<TARGET_PROPERTY:TYPE>,SHARED_LIBRARY>:-nolibc>)

#===------------------------------------------------------------------------===#
# NT Kernel Import Libraries
#===------------------------------------------------------------------------===#

# Path to the NT kernel import libraries generated by libc's build.
# Other runtimes (libunwind, libc++, etc.) reference this variable instead of
# hardcoding libc's internal build layout.
set(NTPOSIX_IMPORT_LIBS_DIR "${CMAKE_BINARY_DIR}/libc/src/__support/OSUtil/windows/import_libs"
  CACHE PATH "Directory containing NT kernel import libraries generated by libc")
...
# -fms-extensions is inherited from WindowsItaniumRules.cmake, which adds it
# to CMAKE_<lang>_FLAGS_INIT for all Windows Itanium / NTPOSIX builds.
...
# sees them and every run aborts with STATUS_DLL_NOT_FOUND (0xC0000135).
#
# Prepend the staging dir to PATH via `cmake -E env --modify` when ninja
# invokes the test command. The recipe is symmetric for __unit__ and
# __hermetic__ paths (hermetic tests link no DLLs but the prefix is a no-op
# cost there and keeps the two COMMAND templates consistent).
#
# We hardcode the staging path because HandleNTPOSIXDefaults runs before
# runtimes/CMakeLists.txt sets LLVM_RUNTIME_OUTPUT_INTDIR; both use the same
# formula.
set(_ntposix_runtime_dll_dir "${CMAKE_BINARY_DIR}/runtime-staging/bin")
# Override PATH with just the runtime DLL dir. We don't need to preserve
# $ENV{PATH} because:
#   - System DLLs (ntdll, kernel32, bcryptprimitives, sspicli) are loaded
#     from the KnownDLLs object directory by name, not via PATH.
#   - Test executables themselves are invoked by absolute path.
#   - Splicing $ENV{PATH} in was attempted but the configure-time expansion
#     injects paths containing spaces (e.g. "C:\Program Files\..."), which
#     break the single-argument PATH=... value when CMake flattens the
#     COMMAND list.
# CMake 3.25's `env --modify PATH=path_list_prepend:...` would sidestep this
# but we support the 3.20 floor.
set(_ntposix_test_env
  ${CMAKE_COMMAND} -E env "PATH=${_ntposix_runtime_dll_dir}")
set(LIBC_UNITTEST_ENV "${_ntposix_test_env}" CACHE STRING
  "NTPOSIX: prepend runtime DLL dir to PATH for unit test runs" FORCE)
set(HERMETIC_TEST_ENV "${_ntposix_test_env}" CACHE STRING
  "NTPOSIX: prepend runtime DLL dir to PATH for hermetic test runs" FORCE)
